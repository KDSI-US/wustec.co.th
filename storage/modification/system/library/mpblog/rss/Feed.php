...
abstract class Feed
{
    ...
    */
     ...
     */
    protected function __construct($version = Feed::RSS2, $oc_registry)
    {

        $this->oc_registry = $oc_registry;
    ...
    * @throws  \LogicException if none of the parameters are set.
    ...
    public function setPagination($nextURL = null, $previousURL = null, $firstURL = null, $lastURL = null)
    {
        if (empty($nextURL) && empty($previousURL) && empty($firstURL) && empty($lastURL))
            throw new \Mpblog\Rss\LogicException('At least one URL must be specified for pagination to work.');
    ...
    */
    public function addGenerator()
    {
        if ($this->version == Feed::ATOM)
            ...
            throw new \Mpblog\Rss\InvalidOperationException('The generator element is not supported in RSS1 feeds.');
    ...
    */
    ...
    {
        if (empty($prefix))
            throw new \Mpblog\Rss\InvalidArgumentException('The prefix may not be emtpy or NULL.');
        if (empty($uri))
            throw new \Mpblog\Rss\InvalidArgumentException('The uri may not be empty or NULL.');
    ...
    */
    public function setChannelElement($elementName, $content, array $attributes = null, $multiple = false)
    {
        if (empty($elementName))
            throw new \Mpblog\Rss\InvalidArgumentException('The element name may not be empty or NULL.');
        if (!is_string($elementName))
            throw new \Mpblog\Rss\InvalidArgumentException('The element name must be a string.');
    ...
    */
    public function printFeed($useGenericContentType = false)
    {
        if (!is_bool($useGenericContentType))
            throw new \Mpblog\Rss\InvalidArgumentException('The useGenericContentType parameter must be boolean.');
    ...
    */
    public function generateFeed()
    {
        if ($this->version != Feed::ATOM && !array_key_exists('link', $this->channels))
            throw new \Mpblog\Rss\InvalidOperationException('RSS1 & RSS2 feeds need a link element. Call the setLink method before this method.');
    ...
    */
    ...
    {
        $Item = new \MpBlog\Rss\Item($this->version, $this->oc_registry);
    ...
    */
    public function addItem(Item $feedItem)
    {
        if ($feedItem->getVersion() != $this->version)
        {
            ...
            throw new \Mpblog\Rss\InvalidArgumentException($msg);
        }
    ...
    */
    ...
    {
        if (empty($encoding))
            throw new \Mpblog\Rss\InvalidArgumentException('The encoding may not be empty or NULL.');
        if (!is_string($encoding))
            throw new \Mpblog\Rss\InvalidArgumentException('The encoding must be a string.');
    ...
    */
    public function setDate($date)
    {
        if ($this->version == Feed::RSS1)
            throw new \Mpblog\Rss\InvalidOperationException('The publication date is not supported in RSS1 feeds.');
        ...
        else if (is_string($date))
        {
            ...
            if ($timestamp === FALSE)
                throw new \Mpblog\Rss\InvalidArgumentException('The given date was not parseable.');

            $date = date($format, $timestamp);
        }
        else
            throw new \Mpblog\Rss\InvalidArgumentException('The given date is not an instance of DateTime, a UNIX timestamp or a date string.');
    ...
    public function setAtomLink($href, $rel = null, $type = null, $hreflang = null, $title = null, $length = null)
    {
        ...
        if ($rel != null) {
            if (!is_string($rel) || empty($rel))
                throw new \Mpblog\Rss\InvalidArgumentException('rel parameter must be a string and a valid relation identifier.');
        ...
        if ($type != null) {
            ...
            if (!is_string($type) || preg_match('/.+\/.+/', $type) != 1)
                throw new \Mpblog\Rss\InvalidArgumentException('type parameter must be a string and a MIME type.');
        ...
        if ($hreflang != null) {
            ...
            if (!is_string($hreflang) || preg_match('/[A-Za-z]{1,8}(-[A-Za-z0-9]{1,8})*/', $hreflang) != 1)
                throw new \Mpblog\Rss\InvalidArgumentException('hreflang parameter must be a string and a valid language code.');

            $data['hreflang'] = $hreflang;
        }
        if ($title != null) {
            if (!is_string($title) || empty($title))
                throw new \Mpblog\Rss\InvalidArgumentException('title parameter must be a string and not empty.');

            $data['title'] = $title;
        }
        if ($length != null) {
            if (!is_int($length) || $length < 0)
                throw new \Mpblog\Rss\InvalidArgumentException('length parameter must be a positive integer.');
            ...
            foreach ($this->channels as $key => $value) {
                ...
                foreach ($value as $linkItem) {
                    ...
                    if (@$attrib['rel'] == 'alternate' && @$attrib['hreflang'] == $hreflang && @$attrib['type'] == $type)
                        throw new \Mpblog\Rss\InvalidOperationException('The feed must not contain more than one link element with a'
    ...
    */
    public function setImage($url, $title = null, $link = null)
    {
        if (!is_string($url) || empty($url))
            throw new \Mpblog\Rss\InvalidArgumentException('url parameter must be a string and may not be empty or NULL.');

        // RSS feeds have support for a title & link element.
        if ($this->version != Feed::ATOM)
        {
            if (!is_string($title) || empty($title))
                throw new \Mpblog\Rss\InvalidArgumentException('title parameter must be a string and may not be empty or NULL.');
            if (!is_string($link) || empty($link))
                throw new \Mpblog\Rss\InvalidArgumentException('link parameter must be a string and may not be empty or NULL.');
    ...
    */
    ...
    {
        if ($this->version != Feed::RSS1)
            throw new \Mpblog\Rss\InvalidOperationException("This method is only supported in RSS1 feeds.");
        if (empty($url))
            throw new \Mpblog\Rss\InvalidArgumentException('The about URL may not be empty or NULL.');
        if (!is_string($url))
            throw new \Mpblog\Rss\InvalidArgumentException('The about URL must be a string.');
    ...
    private function makeHeader()
    {
        ...
        foreach ($prefixes as $prefix) {
            if (!isset($this->namespaces[$prefix]))
                throw new \Mpblog\Rss\InvalidOperationException('Unknown XML namespace prefix: \'' . $prefix . '\'.'
        ...
        if (is_array($tagContent)) {
            foreach ($tagContent as $key => $value) {
                if (is_array($value)) {
                    ...
                    throw new \Mpblog\Rss\InvalidArgumentException("Unknown node-value type for $key");
                }
            }
        } else {
    ...
    private function startItem($about = false)
    {
        ...
        if ($this->version == Feed::RSS2) {
            ...
            if ($about) {
                ...
                throw new \Mpblog\Rss\InvalidOperationException("Missing data for about attribute. Call setChannelAbout method.");
            }
        } elseif ($this->version == Feed::ATOM) {
            $out .= "<entry>" . PHP_EOL;
        }
