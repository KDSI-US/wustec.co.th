...
class XMLReaderIterator implements Iterator, XMLReaderAggregate
{
    ...
    public function moveToNextElementByName($name = null)
    {
        while (self::moveToNextElement()) {
            ...
            self::next(); //@gkd, try to remove this line if issue with iteration
        }
        ;
...
class XMLElementIterator extends XMLReaderIterator
{
    ...
    {
      // @gkd
      if (is_null($this->current()) && $name == 'readOuterXML') {
        return '';
      }
...
class XMLElementXpathFilter extends XMLReaderFilterBase
{
    ...
    */
    ...
    public function accept()
    {
        ...
        $buffer = str_replace('xmlns:', 'ns', $buffer); //@gkdfix trick the namespaces in order to process correctly the xpath
        
        if ($res = simplexml_load_string($buffer, "SimpleXMLElement" , LIBXML_NOERROR )) {
          $result = $res->xpath($this->expression);
        } else {
          $result = array();
        }
