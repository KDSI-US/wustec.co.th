...
final class GkdLoader {
    ...
    // generate issues if admin needs to load again some admin template
    // $registry->set('load', $this);
	...
	public function model($route) {
		...
		if (!$this->registry->has('model_' . str_replace(array('/', '-', '.'), array('_', '', ''), $route))) {
			...
			if (is_file($file)) {
        if (isset($vqmod)) {
          function_exists('modification') ? include_once($vqmod->modCheck(modification($file))) : include_once($vqmod->modCheck($file));
        } else if (class_exists('VQMod')) {
          function_exists('modification') ? include_once(\VQMod::modCheck(modification($file))) : include_once(\VQMod::modCheck($file));
        } else {
          function_exists('modification') ? include_once(modification($file)) : include_once($file);
        }
    ...
    if (substr($tpl_file, -4) !== '.tpl') {
      $tpl_file .= '.tpl';
    }
    
    if (version_compare(VERSION, '3', '>=')) {
      $tpl_file = preg_replace('/[^a-zA-Z0-9_\/\.]/', '', (string)$route);
      
      if (strpos($route, '.twig')) {
        $tpl_file = str_replace('.twig', '', $tpl_file);
        $template = new Template('gkd_twig', $this->registry);
      } else {
