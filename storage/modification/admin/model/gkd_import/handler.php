...
class ModelGkdImportHandler extends Model {
  ...
  public function customerHandler($value, $config) {
    ...
    // get by id
    if (is_numeric($value)) {
      $query = $this->db->query("SELECT customer_id, email FROM " . DB_PREFIX . "customer WHERE customer_id = '" . (int) $value . "'")->row;
    } else {
      $query = $this->db->query("SELECT customer_id, email FROM " . DB_PREFIX . "customer WHERE email = '" . $value . "'")->row;
    }
    
    if (!empty($query['customer_id'])) {
      $customer_name = $query['email'];
  ...
  public function optionHandler($field, $config, $line, $product_id, $is_option = false) {
    $return_values = $values_array = $summed_quantity = $header_keys = $header_keys_number = array();
    
    if (!empty($config['include_option'])) {
      $config['include_option'] = @html_entity_decode($config['include_option'], ENT_QUOTES, 'UTF-8');
    }
    ...
    // combine options mode
    if (!empty($config['include_option']) && strpos($config['include_option'], '~')) {
      $newValuesArray = $newHeaderArray = $newHeaderKeyNumber = [];
      
      list($optParent, $optChild) = explode('~', $config['include_option']);
      
      if (strpos($optParent, ':')) {
        list($optName, $optParent) = explode(':', $optParent);
      } else {
        $optName = $optParent . ' - ' . $optChild;
      }
      
      foreach ($header_keys as $k => $headKey) {
        if ($headKey == $optParent && !empty($values_array[$k])) {
          foreach ($values_array as $valKey => $val) {
            if ($header_keys[$valKey] == $optChild) {
              $newValuesArray[] = $optName . ':' . $values_array[$k] . ' - ' . $val;
              $newHeaderArray[] = $optName;
              $newHeaderKeyNumber[] = $k;
            }
          }
        }
      }
      ...
      $values_array = $newValuesArray;
      $header_keys = $newHeaderArray;
      $header_keys_number = $newHeaderKeyNumber;
    }
    ...
    foreach ($values_array as $current_key => &$value) {
      ...
      foreach ($optionFields as $currentOptType) {
        ${'option_'.$currentOptType.'_arr'} = array();
      }
      ...
      if (!in_array($this->filetype, array('xml', 'json')) && !empty($config['option_fields']) && !$value) {
        ...
        */
        ...
        foreach ($config['option_fields'] as $extraOptionFieldName => $extraOptionFieldVal) {
          if (!in_array($extraOptionFieldName, array('product_option_value_id', 'product_option_id', 'product_id', 'option_id', 'option_value_id', 'quantity', 'subtract', 'price', 'price_prefix', 'points', 'points_prefix', 'weight', 'weight_prefix'))) {
            $optionFields[] = $extraOptionFieldName;
          }
        }
        */
        ...
        foreach ($optionFields as $currentOptType) {
          if (isset($config['option_fields'][$currentOptType]) && is_scalar($config['option_fields'][$currentOptType]) && !empty($line[$config['option_fields'][$currentOptType]])) {
          ...
          // handle option separator
          if (${'option_'.$currentOptType} && !empty($config['option_separator'])) {
            ${'option_'.$currentOptType.'_arr'} = explode($config['option_separator'], ${'option_'.$currentOptType});
          }
        }
        ...
        if (isset($config['option_fields']['value']) && is_array($config['option_fields']['value'])) {
          ...
          foreach ($config['option_fields']['value'] as $lang_id => $optValueField) {
            if (($optValueField == '[current]' || $optValueField == '.' || $optValueField == '') && is_string($value)) {
              ...
              foreach ($valFields as $valFieldKey => $valField) {
                if (!empty($line[$valField]) && substr($valField, 0, 2) !== './') {
                  foreach ((array) $line[$valField] as $k => $v) {
                    if (!isset($composed[$k])) {
                      $composed[$k] = '';
                    }
                    
                    if (!empty($v)) {
                      $composed[$k] .= (!empty($composed[$k]) ? '|' : '') . $v;
                    }
                  }
                } else {
                  if (substr($valField, 0, 2) == './') {
                  ...
                  if (!isset($composed[$current_key])) {
                    $composed[$current_key] = '';
                  }
                  
                  if (substr($valField, -2) == '[]') {
                    // array detect mode
                    $valField = substr($valField, 0, -2);
                    $arrValues = (array) $this->getArrayPath($value, $valField, false);
                    foreach($arrValues as $subArrKey => $arrVal) {
                      $composed[$current_key.'-'.$subArrKey] = (!empty($composed[$current_key]) ? $composed[$current_key] . '|' : '') . $arrVal;
                    }
                    
                    unset($composed[$current_key]);
                  } else {
                    $composed[$current_key] .= (!empty($composed[$current_key]) ? '|' : '') . $this->getArrayPath($value, $valField);
                  }
                }
              }
              
              foreach ($composed as $comp) {
                $currentVal[] = trim($comp, '|');
              }
            } else if (strpos($optValueField, '+')) {
              $valFields = explode('+', $optValueField);
              $currentVal = array();
              ...
              foreach ($valFields as $k => $valField) {
                if (!empty($line[$valField]) && substr($valField, 0, 2) !== './') {
                  ...
                  if (substr($valField, -2) == '[]') {
                    ...
                    foreach($arrValues as $subArrKey => $arrVal) {
                      $currentVal[] = (!empty($composed[$current_key]) ? $composed[$current_key] . '|' : '') . $arrVal;
                    }
                  } else {
                    $currentVal[] = $this->getArrayPath($value, $valField);
                  }
        ...
        */
        ...
        foreach ($optionFields as $currentOptType) {
          $currentVal = NULL;
           ...
           if (isset($config['option_fields'][$currentOptType]) && substr($config['option_fields'][$currentOptType], -2) == '[]') {
            // array detect mode
            $currentVal = array();
            $valField = substr($config['option_fields'][$currentOptType], 0, -2);
            $arrValues = (array) $this->getArrayPath($value, $valField, false);
            foreach($arrValues as $subArrKey => $arrVal) {
              $composed[$current_key.'-'.$subArrKey] = (!empty($composed[$current_key]) ? $composed[$current_key] . '|' : '') . $arrVal;
            }
          ...
          } else if (isset($config['option_fields'][$currentOptType]) && strpos($config['option_fields'][$currentOptType], '+')) {
            $currentVal = 0;
            
            foreach (explode('+', $config['option_fields'][$currentOptType]) as $subPath) {
              $val = $this->getArrayPath($value, $subPath);
              
              if (is_numeric($val)) {
                $currentVal += $this->getArrayPath($value, $subPath);
              }
            }
            
            ${'option_'.$currentOptType} = $currentVal;
          } else if (empty($value) && isset($config['option_fields'][$currentOptType]) && !empty($line[$config['option_fields'][$currentOptType]])) {
          ...
          if (!empty($currentVal) && !empty($config['option_separator']) && strpos($currentVal, $config['option_separator'])) {
            $currentVal = explode($config['option_separator'], $currentVal);
          }
        ...
        foreach ($option_values as $i => $optVal) {
          if (!empty($option_quantity)) {
            $summed_quantity[$optVal] = isset($summed_quantity[$optVal]) ? $summed_quantity[$optVal] + (isset($option_quantity_arr[$i]) ? $option_quantity_arr[$i] : $option_quantity) : (isset($option_quantity_arr[$i]) ? $option_quantity_arr[$i] : $option_quantity);
          }
        }
            ...
            foreach ($config['option_fields_default']['name'] as $lang_id => $optNameField) {
              if (empty($option_names_ml[$i][$lang_id])) {
                ...
                if (count($option_values) > 1) {
                  if (isset($optNames[$i])) {
                    $option_names_ml[$i][$lang_id] = $optNames[$i];
                  } else if ($lastVal = end($optNames)) {
                    $option_names_ml[$i][$lang_id] = $lastVal;
                  } else if (!empty($optNames[0])) {
                    $option_names_ml[$i][$lang_id] = $optNames[0];
                    $option_names_ml[$i][$lang_id] = isset($optNames[$i]) ? $optNames[$i] : (!empty($optNames[0]) ? $optNames[0] : 'Size');
                  } else {
      ...
      if (!empty($config['filters_from_options'])) {
        if ($option_quantity || empty($option_subtract)) {
          foreach ($option_values as $i => $optVal) {
            if (!empty($config['include_option'])) {
            ...
            $config['columns']['product_filter'][$option_names[$i].':'.serialize($optVal)] = array('group' => $option_names[$i], 'name' => $optVal);
          }
        }
      }
      
      if (!empty($config['attributes_from_options'])) {
        if ($option_quantity || empty($option_subtract)) {
          foreach ($option_values as $i => $optVal) {
            if (!empty($config['include_option'])) {
              if (!in_array($option_names[$i], explode('|', $config['include_option']))) {
            ...
            $config['columns']['product_attribute'][$option_names[$i].':'.serialize($optVal)] = array('name' => $option_names[$i], 'value' => $optVal);
          }
        }
      }
      
      // convert prices with commas
      if (empty($option_price) && !empty($option_price_arr[0])) {
        $option_price = $option_price_arr[0];
      }
      
      $option_price = $this->tool->floatValue($option_price);
      
      foreach ($option_price_arr as $optPriceKey => $optPriceVal) {
        $option_price_arr[$optPriceKey] = $this->tool->floatValue($optPriceVal);
      }
      
      // calculate price difference
      if (!empty($config['option_price_mode']) && $option_price && !(isset($option_price_prefix) && $option_price_prefix == '=')) {
        if (!$is_option && !empty($config['columns']['price'])) {
          ...
          } else if ($this->simulation && $is_option) {
            $product_price = 0; // will be calculated on real import
          } else {
            $product_price = $config['columns']['price'];
          }
        }
        ...
        foreach ($option_price_arr as $optPriceKey => $optPriceVal) {
          $option_price_arr[$optPriceKey] = (float) $optPriceVal - (float) $product_price;
        }
      }
      ...
      if ($this->simulation) {
        foreach ($option_values as $i => $option_value) {
          ...
          if (!empty($config['exclude_option_value'])) {
            if (in_array($option_value, explode('|', $config['exclude_option_value']))) {
              continue;
            }
          }
          ...
          if (is_array($option_value)) {
            ...
            foreach (array('prod_opt_val_id', 'quantity', 'price', 'ean', 'sku', 'upc', 'model', 'weight') as $dispOptType) {
              if (!empty(${'option_'.$dispOptType.'_arr'}[$i]) || !empty(${'option_'.$dispOptType})) {
                $optPreviewData[] = str_replace('quantity', 'qty', $dispOptType) . ': '. (count($option_values) > 1 && !empty(${'option_'.$dispOptType.'_arr'}[$i]) ? ${'option_'.$dispOptType.'_arr'}[$i] : ${'option_'.$dispOptType});
              }
            }
      ...
      foreach ($option_values as $i => $option_value) {
        ...
        */
        ...
        foreach ($optionFields as $currentOptType) {
          if (count($option_values) > 1 && isset(${'option_'.$currentOptType.'_arr'}[$i])) {
        ...
        */
        ...
        if (in_array($option_type, array('select', 'radio', 'checkbox', 'image'))) {
          ...
          $return_values[$option_name]['product_option_value'][serialize($option_value).$option_weight] = array(
            ...
            'price' => !empty($option_price) ? abs((float) $option_price) : '',
            'price_prefix' => $option_price_prefix ,
            'points' => !empty($option_points) ? abs((float) $option_points) : '',
            'points_prefix' => !empty($option_points) && ($option_points < 0) ? '-' : '+',
            'weight' => !empty($option_weight) ? abs((float) $option_weight) : '',
          ...
          foreach ($config['option_fields'] as $extraOptionFieldName => $extraOptionFieldVal) {
            if (!in_array($extraOptionFieldName, array('product_option_value_id', 'product_option_id', 'product_id', 'option_id', 'option_value_id', 'quantity', 'subtract', 'price', 'price_prefix', 'points', 'points_prefix', 'weight', 'weight_prefix'))) {
              $return_values[$option_name]['product_option_value'][serialize($option_value).$option_weight][$extraOptionFieldName] = isset(${'option_'.$extraOptionFieldName}) ? ${'option_'.$extraOptionFieldName} : '';
            }
          }
          */
        } else {
  ...
  public function discountHandler($field, $config) {
    ...
    // advanced mode
    if (isset($config['columns'][$field][0]) && is_array($config['columns'][$field][0]) && (!is_scalar($config['columns'][$field][0]) || $config['columns'][$field][0] == '') && in_array($this->filetype, array('xml', 'json'))) {
      foreach ($config['columns_bindings']['discountByCustomerGroup'] as $customer_group_id => $groupData) {
        foreach ($config['columns'][$field][0] as $value) {
          $currentArray = array(
            'customer_group_id' => $customer_group_id,
          );
          
          foreach ($groupData as $key => $val) {
            $currentVal = $this->getArrayPath(is_array($value) ? $value : $config['columns'], $config['columns_bindings']['discountByCustomerGroup'][$customer_group_id][$key]);
            
            if (!empty($currentVal)) {
              $currentArray[$key] = $currentVal;
            } else if (!empty($config['defaults']['discountByCustomerGroup'][$customer_group_id][$key])) {
              $currentArray[$key] = $config['defaults']['discountByCustomerGroup'][$customer_group_id][$key];
            } else {
              $currentArray[$key] = '';
            }
          }
          
          if (!empty($currentArray['quantity']) && !empty($currentArray['price'])) {
            $return_values[] = $currentArray;
          }
        }
      }
    } else if (!empty($config['columns']['discountByCustomerGroup'])) {
  ...
  public function specialHandler($field, $config) {
    ...
    if (!empty($config['columns']['specialByCustomerGroup'])) {
      foreach ($config['columns']['specialByCustomerGroup'] as $customer_group_id => $groupData) {
        if (isset($config['columns']['price']) && isset($groupData['price']) && $config['columns']['price'] == $this->tool->floatValue($groupData['price'])) {
        ...
        // string mode autodetect
          ...
          foreach ($this->customer_groups as $customer_group_id) {
            ...
            $return_values[] = array(
              ...
              'date_start' => (!empty($config['defaults']['specialByCustomerGroup'][$customer_group_id]['date_start']) ? date('Y-m-d', strtotime($config['defaults']['specialByCustomerGroup'][$customer_group_id]['date_start'])) : '2000-01-01'),
              'date_end' => (!empty($config['defaults']['specialByCustomerGroup'][$customer_group_id]['date_end']) ? date('Y-m-d', strtotime($config['defaults']['specialByCustomerGroup'][$customer_group_id]['date_end'])) : '2100-01-01'),
            );
          }
        }
      }
  ...
  public function filterHandler($field, $config) {
    ...
    foreach ((array) $config['columns'][$field] as $key => $value) {
      ...
      if (is_array($value) && isset($value['group']) && isset($value['name'])) {
        $values_array[] = $value;
      } else if (is_array($value)) {
        $values_array = array_merge($values_array, $value);
    ...
    if (empty($config['filter_fields']['name'][$this->config->get('config_language_id')])) {
    ...
    foreach ($values_array as $current_key => &$value) {
      // gkd attribute
      if (!empty($value['gkdAttr'])) {
        foreach (array('group', 'name') as $attribute_type) {
          ${'filter_'.$attribute_type.'_ml'} = $value[$attribute_type];
      ...
      // xml advanced values
      } else if ($this->filetype == 'xml' && !empty($config['filter_fields']['name'][$this->config->get('config_language_id')])) {
        foreach (array('group', 'name') as $attribute_type) {
          foreach ($languages as $language) {
            if (!empty($config['filter_fields'][$attribute_type][$language['language_id']])) {
              $currentValue = $this->getArrayPath($value, $config['filter_fields'][$attribute_type][$language['language_id']], false);
              
              if (is_scalar($currentValue)) {
                ${'filter_'.$attribute_type.'_ml'}[$language['language_id']] = $currentValue;
              } else if (is_array($currentValue)) {
                if ($attribute_type == 'group') {
                  $currentGroupArray = $currentValue;
                } else {
                  foreach ($currentValue as $subAttrKey => $subAttrVal) {
                    $values_array['gkd'.$current_key.$subAttrKey]['gkdAttr'] = true;
                    $values_array['gkd'.$current_key.$subAttrKey][$attribute_type][$language['language_id']] = $subAttrVal;
                    $values_array['gkd'.$current_key.$subAttrKey]['group'][$language['language_id']] = isset($currentGroupArray[$subAttrKey]) ? $currentGroupArray[$subAttrKey] : '';
                  }
                }
              }
            }
      ...
      // import by filter id
      ...
      } else if (is_array($value) && isset($value['group']) && isset($value['name'])) {
        $filter_group = $value['group'];
        ...
        if (!isset($values)) {
          // get column header
          $column_headers = (array) json_decode(base64_decode($config['column_headers']));
          if ($this->filetype == 'xml' && empty($column_headers[ $column_bindings[$current_key] ])) {
            $filter_group = basename(str_replace('_', ' ', $column_bindings[$current_key]));
            $filter_name = $value;
          } else if (!empty($column_headers[ $column_bindings[$current_key] ])) {
  ...
  public function attributeHandler($field, $config, $line) {
    ...
    for ($i = 0; $i < count($values_array); ++$i) {
      ...
      if ((!is_scalar($values_array[$i]) || $values_array[$i] == '') && !in_array($this->filetype, array('xml', 'json')) && !empty($config['attribute_fields'])) {
        foreach ($config['attribute_fields'] as $attrFieldKey => $attribute_fields) {
          foreach (array('group', 'name', 'value') as $attribute_type) {
            ...
            foreach ($languages as $language) {
              if (isset($attribute_fields[$attribute_type][$language['language_id']]) && $attribute_fields[$attribute_type][$language['language_id']] !== '') {
      ...
      // xml advanced values
              ...
              // get path
                ...
                if ($attribute_type == 'value' && strpos($config['attribute_fields']['value'][$language['language_id']], '+')) {
                  ...
                  foreach (explode('~', $config['attribute_fields'][$attribute_type][$language['language_id']]) as $k => $v) {
                    $resVal = $this->getArrayPath((is_array($value) ? $value : $line), $v, false);
                    
                    if (is_array($resVal)) {
                      foreach ($resVal as $k => $v) {
                        ${'multi_attribute_'.$attribute_type.'_ml'}[$language['language_id']][$k] = $v;
                      }
                    } else {
                      // group or name refers to an array, try to get key corresponding to current value key
                      if (is_array($resVal) && array_key_exists($current_key, $resVal)) {
                        ${'attribute_'.$attribute_type.'_ml'}[$language['language_id']][$current_key] = $resVal[$current_key];
                      } else {
                        //${'attribute_'.$attribute_type.'_ml'}[$language['language_id']][$current_key] = $resVal;
                        ${'attribute_'.$attribute_type.'_ml'}[$language['language_id']] = $resVal;
                      }
                    }
      ...
      } else if (is_array($value) && isset($value['group'])) {
        ...
        if (!empty($config['attr_mode'])) {
          $values = explode(':', $value, (int) $config['attr_mode']);
        } else {
        ...
        if (count($values) == 1) {
          ...
          if (!empty($config['attribute_fields_default']) && is_array($config['attribute_fields_default'])) {
            $firstDefaultAttributeFields = $config['attribute_fields_default'];
            foreach (array('group', 'name', 'value') as $attribute_type) {
              if (isset($firstDefaultAttributeFields[$attribute_type]) && array_filter($firstDefaultAttributeFields[$attribute_type])) {
                foreach ($languages as $language) {
                  if (!empty($firstDefaultAttributeFields[$attribute_type][$language['language_id']])) {
                    $custom_data = explode('|', $firstDefaultAttributeFields[$attribute_type][$language['language_id']]);
        ...
        } else if (count($values) == 2 || count($values) == 3) {
          if (!empty($config['attribute_fields_default']) && is_array($config['attribute_fields_default'])) {
            $firstDefaultAttributeFields = $config['attribute_fields_default'];
          ...
          if (count($values) == 2) {
            ...
            $attribute_group = $values[0] ? $values[0] : $attribute_group;
    ...
    */
    ...
    if (empty($processed) && is_array($values_array)) {
      foreach ($value as $name => $val) {
        $attribute_group = isset($value['group']) ? $value['group'] : 'Default';
        $attribute_name = isset($name) ? $name : 'Attribute';
        $attribute_value = isset($val) ? $val : '';
    ...
    if (true) {
      foreach ($attributes_array as $current_key => &$attr) {
        foreach ($attributes_array as $subkey => $subAttr) {
          if ($subkey != $current_key && $attr['attribute_group'] == $subAttr['attribute_group'] && $attr['attribute_name'] == $subAttr['attribute_name'] && $attr['attribute_value'] != $subAttr['attribute_value'] && trim($attr['attribute_value']) !== '' && trim($subAttr['attribute_value']) !== '') {
            ...
            if (is_array($attr['attribute_value_ml'])) {
              foreach ($attr['attribute_value_ml'] as $language_id => &$attrValMl) {
                if (isset($subAttr['attribute_value_ml'][$language_id]) && trim($subAttr['attribute_value_ml'][$language_id]) !== '') {
    ...
    foreach ($attributes_array as $current_key => $attributeValues) {
      ...
      if (!empty($config['filters_from_attributes'])) {
        $config['columns']['product_filter'][] = array('group' => $attribute_name, 'name' => $attribute_value);
      }
      if ($attribute_group == '') {
        $attribute_group = 'Default';
      }
      ...
      if (!empty($attribute_group)) {
        $attr_group = $this->db->query("SELECT attribute_group_id FROM " . DB_PREFIX . "attribute_group_description WHERE name = '" . $this->db->escape($this->request->clean($attribute_group)) . "'")->row;
        ...
        //  group not exists - create
          ...
          if (version_compare(VERSION, '2', '>=')) {
            ...
            $attribute_group_id = $this->model_catalog_attribute_group->addAttributeGroup($this->request->clean($attr_group_data));
          } else {
            $this->load->model('gkd_import/attribute');
            $attribute_group_id = $this->model_gkd_import_attribute->addAttributeGroup($this->request->clean($attr_group_data));
          }
        }
      }
      ...
      if ($this->simulation) {
        if (trim($attribute_value)) {
          ...
          if (is_numeric($attribute_name)) {
            $attrExists = $this->db->query("SELECT attribute_id, name FROM " . DB_PREFIX . "attribute_description WHERE attribute_id = '" . $this->db->escape($this->request->clean($attribute_name)) . "'")->row;
            ...
            if (!empty($attribute_group_id)) {
              $attrExists = $this->db->query("SELECT d.attribute_id, d.name FROM " . DB_PREFIX . "attribute_description d LEFT JOIN " . DB_PREFIX . "attribute a ON (a.attribute_id = d.attribute_id) WHERE d.name = '" . $this->db->escape($this->request->clean($attribute_name)) . "' AND  a.attribute_group_id = '" . (int) $attribute_group_id . "'")->row;
            } else {
              $attrExists = $this->db->query("SELECT attribute_id, name FROM " . DB_PREFIX . "attribute_description WHERE name = '" . $this->db->escape($this->request->clean($attribute_name)) . "'")->row;
            }
          }
      ...
      if (is_numeric($attribute_name)) {
        $attrExists = $this->db->query("SELECT attribute_id FROM " . DB_PREFIX . "attribute_description WHERE attribute_id = '" . $this->db->escape($this->request->clean($attribute_name)) . "'")->row;
      } else {
        if (!empty($attribute_group_id)) {
          $attrExists = $this->db->query("SELECT d.attribute_id FROM " . DB_PREFIX . "attribute_description d LEFT JOIN " . DB_PREFIX . "attribute a ON (a.attribute_id = d.attribute_id) WHERE d.name = '" . $this->db->escape($this->request->clean($attribute_name)) . "' AND  a.attribute_group_id = '" . (int) $attribute_group_id . "'")->row;
        } else {
          $attrExists = $this->db->query("SELECT attribute_id FROM " . DB_PREFIX . "attribute_description WHERE name = '" . $this->db->escape($this->request->clean($attribute_name)) . "'")->row;
        }
      }
      ...
      // not exists - create
        ...
        $attr_data['attribute_group_id'] = $attribute_group_id;
        ...
        if (version_compare(VERSION, '2', '>=')) {
          ...
          $attribute_id = $this->model_catalog_attribute->addAttribute($this->request->clean($attr_data));
        } else {
          $this->load->model('gkd_import/attribute');
  ...
  public function imageHandler($field, $config, $multiple = false, $item_id = NULL, $get_path = false, $main_image = false) {
    ...
    foreach ((array) $config['columns'][$field] as $images) {
      ...
      if (!empty($config['multiple_separator']) && is_string($main_image)) {
        $main_image = explode(@html_entity_decode($config['multiple_separator'], ENT_QUOTES, 'UTF-8'), $main_image);
      }
      ...
      if ($multiple && isset($images[0]) && $main_image == $images[0]) {
        if (!(substr($config['image_location'], 0, 4) != 'http' && !empty($images[0]) && substr($images[0], 0, 4) != 'http' && is_dir(DIR_IMAGE . trim($images[0], '/')))) {
          array_shift($images);
        }
      }
      ...
      foreach ((array) $images as $i => $image) {
        ...
        if (!empty($config['image_number']) && $i >= (int) $config['image_number']) continue;
        ...
        if ($config['image_download'] && $image) {
          ...
          if (!empty($config['image_name_param'])) {
            ...
            foreach (explode('|', $config['image_name_param']) as $image_name_param) {
              if (!empty($query_params[$image_name_param])) {
                $file_info = pathinfo($query_params[$image_name_param]);
                break;
              }
            }
          }
          
          if (!isset($file_info['dirname']) || $file_info['dirname'] == '.') {
            $file_info['dirname'] = '';
          }
          ...
          if (!empty($config['image_name_param']) && !in_array(strtolower($file_info['extension']), array('gif', 'jpg', 'jpeg', 'png', 'webp'))) {
          ...
          if (substr($config['image_location'], 0, 4) != 'http' && !empty($filename) && substr($filename, 0, 4) != 'http' && is_dir(DIR_IMAGE . trim($filename, '/'))) {
            foreach (glob(DIR_IMAGE . trim($filename, '/') . '/*') as $i => $imgFile) {
              if (!in_array(strtolower(pathinfo($imgFile, PATHINFO_EXTENSION)), array('gif', 'jpg', 'jpeg', 'png', 'webp'))) continue;
              
              if ($i == 0 && $main_image == $filename) continue;
              
              if ($this->simulation) {
                if (!$multiple) {
                  return str_replace(DIR_IMAGE, HTTP_CATALOG.'image/', $imgFile);
                } else {
                  if (!empty($imgFile)) {
                    $image_array[$imgFile] = str_replace(DIR_IMAGE, HTTP_CATALOG.'image/', $imgFile);
                  }
                }
              } else {
                if (!$multiple) {
                  return str_replace(DIR_IMAGE, '', $imgFile);
                } else {
                  if (!empty($imgFile)) {
                    $image_array[$imgFile] = array(
                      'image' => str_replace(DIR_IMAGE, '', $imgFile),
                      'sort_order' => $sort_order++,
                    );
                  }
                }
        ...
        if (!$multiple) {
          if (!$filename && count((array) $config['columns'][$field]) > 1) {
            continue;
          }
      ...
      foreach ((array) $downloads as $download) {
        if ((!is_scalar($download) || $download == '') && in_array($this->filetype, array('xml', 'json')) && !empty($config['download_fields']['value'])) {
          if (!empty($config['download_fields']['value'])) {
            $download = $this->getArrayPath(is_array($download) ? $download : $config['columns'], $config['download_fields']['value']);
          }
        ...
        if (strpos($download, 'https://www.dropbox.com/') === 0 && !strpos($download, 'dl=1')) {
          $download = $download . '?dl=1';
        }
        
        if (strpos($download, '.google.com/') && !strpos($download, 'export') && !strpos($download, 'output')) {
          if (strpos($download, 'docs.google.com/spreadsheets')) {
            // remove params
            if (strstr($download, '?', true)) {
              $download = strstr($download, '?', true);
            }
            
            $download = str_replace('/edit', '/export?format=csv', $download);
          } else {
            $linkData = explode('/', $download);
            $fileCode = false;
            ...
            if ($fileCode) {
              $download = 'https://drive.google.com/uc?id='.$fileCode.'&export=download';
            } else {
              sleep(1);
        ...
        if ($download) {
          ...
          */
          ...
          if (empty($file_info['extension'])) {
            ...
            if (empty($file_info['extension']) && $redirect != $download) {
              $ch = curl_init();
              curl_setopt($ch, CURLOPT_URL, trim($redirect));
              curl_setopt($ch, CURLOPT_HEADER, true);
              curl_setopt($ch, CURLOPT_NOBODY, true);
              curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
              curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
          ...
          if (!empty($file_info['extension'])) {
            ...
            $filename = urldecode($file_info['filename']);
          }
    ...
    foreach ((array) $config['columns'][$field] as $language_id => $sectionDownloads) {
      ...
      foreach ($sectionDownloads as $k => $downloads) {
        ...
        foreach ((array) $downloads as $download) {
          ...
          if (substr_count($download, ':') > 1) {
            list($displayName, $download) = explode(':', $download, 2);
          }
          
          if ((!is_scalar($download) || $download == '') && in_array($this->filetype, array('xml', 'json')) && !empty($config['public_download_fields']['value'])) {
            if (!empty($config['public_download_fields']['value'])) {
              $download = $this->getArrayPath(is_array($download) ? $download : $config['columns'], $config['public_download_fields']['value']);
            }
          }
          ...
          if ($download) {
            ...
            */
            ...
            if ($this->simulation) {
              $downloadArray[$language_id][] = $filename . ($displayName ? ' ('.$displayName.')' : '');
              continue;
            }
          ...
          if (!empty($filename)) {
            ...
            if (!empty($displayName)) {
              $downloadArray['mask'][$language_id][] = $displayName;
            }
          }
        }
      }
  ...
  */
  ...
  public function manufacturerHandler($config, &$noDeleteBrands) {
      ...
      if ($this->simulation) {
        if (!empty($query['name'])) {
          if ($query['manufacturer_id'] && !empty($config['delete']) && $config['delete'] == 'missing_brand') {
            ...
            $noDeleteBrands[] = $query['manufacturer_id'];
          }
    ...
    if (is_numeric($cat_name)) {
      ...
      if (count($query) === 1) {
        if ($this->simulation) {
          ...
          $simu_text = '['.$query[0]['category_id'].'] ' . $query[0]['name'];
        } else {
        ...
        if ($config['import_type'] == 'category') {
          $storeIds = isset($config['columns']['category_store']) ? $config['columns']['category_store'] : (isset($config['defaults']['category_store']) ? $config['defaults']['category_store'] : array(0));
        } else {
          $storeIds = isset($config['columns']['product_store']) ? $config['columns']['product_store'] : (isset($config['defaults']['product_store']) ? $config['defaults']['product_store'] : array(0));
        }
            ...
            if (!$this->config->get('mlseo_enabled')) {
              foreach ($storeIds as $storeId) {
                foreach ($languages as $language) {
                  ...
                  $cat_data['category_seo_url'][$storeId][$language['language_id']] = $seo;
                }
              }
            }
  ...
  public function categoryHandler($field, $config, $product_id, $force_parent_id = false, $ids_field = false) {
    ...
    foreach ((array) $config['columns'][$field] as $key => $categories) {
      ...
      } else if (is_array($categories) && !empty($config['columns']['sub_'.$field][$key]) && is_array($config['columns']['sub_'.$field][$key])) {
        // add subcat array to cat array
        foreach ($categories as $tempCatKey => $tempCatVal) {
          foreach ($config['columns']['sub_'.$field][$key] as $subcat) {
            if (is_string($categories[$tempCatKey]) && is_string($subcat[$tempCatKey])) {
              $categories[$tempCatKey] .= @html_entity_decode($config['subcategory_separator'], ENT_QUOTES, 'UTF-8') . $subcat[$tempCatKey];
            }
          }
        }
      }
      ...
      $colBindingExist = false;
      
      if (is_array($categories) && count($categories) == 1 && !empty($categories[0])) {
      ...
      if (is_array($categories)) {
        ...
        foreach ($categories as $catToBind) {
          if (is_scalar($catToBind) && !empty($config['col_binding'][md5($catToBind)])) {
            $colBindingExist = true;
            
            if (!empty($config['include_subcat'])) {
              $addCategories = array();
              
              foreach ((array) $config['col_binding'][md5($catToBind)] as $colBindId) {
                ...
                $config['col_binding'][md5($catToBind)] = array_merge($config['col_binding'][md5($catToBind)], $addCategories);
              }
            }
          
            $tempCategories[] = $config['col_binding'][md5($catToBind)];
          }
        }
        
        if ($this->simulation) {
          foreach ((array) $tempCategories as $subTempCategories) {
            foreach ($subTempCategories as $colBindId) {
              ...
              if (!empty($query['category_id'])) {
                ...
                $values[] = '<span title="Binding">[*'.$cat_id.']</span> ' . $cat_name;
              } else {
                ...
                $this->tool->log(array(
                  ...
                  'status' => 'error',
                  'title' => $this->language->get('warning'),
                  'msg' => $this->language->get('warning_category_id') . ' - Binding : '.$colBindId,
                ));
              }
            }
          }
          ...
          foreach ((array) $tempCategories as $subTempCategories) {
            $values = array_merge($subTempCategories, $values);
          }
        }
      ...
      } else if (is_scalar($categories) && !empty($config['col_binding'][md5($categories)])) {
        $colBindingExist = true;
      ...
      if (!$colBindingExist) {
        if (isset($config['col_binding_mode']) && $config['col_binding_mode'] == '2') {
          ...
          if ($this->simulation) {
            ...
            if (!empty($query['category_id'])) {
              ...
              $this->tool->log(array(
                ...
                'status' => 'error',
                'title' => $this->language->get('warning'),
                'msg' => $this->language->get('warning_category_id') . ': '.$category,
              ));
            }
          } else {
            $values[] = $category;
          ...
          foreach ($full_categories as $cat_name) {
            ...
            if (empty($cat_exists['category_id'])) {
              ...
              if (!$this->config->get('mlseo_enabled')) {
                foreach ($storeIds as $storeId) {
                  foreach ($languages as $language) {
                    $cat_data['category_seo_url'][$storeId][$language['language_id']] = !empty($cat_name_ml[$language['code']]) ? $this->tool->urlify($cat_name_ml[$language['code']]) : $this->tool->urlify($cat_name);
                  }
                }
              }
    ...
    if (!empty($currentCats)) {
      foreach ($currentCats as $currentCat) {
        if ($this->simulation) {
          $query = $this->db->query("SELECT name, category_id FROM " . DB_PREFIX . "category_description WHERE category_id = '" . (int) $currentCat['category_id'] . "'")->row;
          if (!empty($query['category_id'])) {
            $values[] = '<span title="Preserve existing">[#'.$query['category_id'].']</span> ' . $query['name'];
          }
        } else {
          $values[] = $currentCat['category_id'];
        }
      }
    }
  ...
  public function recurringHandler($field, $config) {
    $values_return = $values_array = array();
    ...
    foreach ($values_array as $value) {
      if (strpos($value, ':')) {
        list($group, $recurring) = explode(':', $value);
      } else {
        $recurring = $value;
      }
      ...
      if (is_numeric($recurring)) {
        $recurringQuery = $this->db->query("SELECT * FROM " . DB_PREFIX . "recurring WHERE recurring_id = '" . (int) $recurring . "'")->row;
      } else {
        $recurringQuery = $this->db->query("SELECT * FROM " . DB_PREFIX . "recurring_description WHERE name = '" . $this->db->escape($recurring) . "'")->row;
      }
      
      // customer group
      if (!empty($group)) {
        if (is_numeric($group)) {
          $query = $this->db->query("SELECT customer_group_id, name FROM " . DB_PREFIX . "customer_group_description WHERE customer_group_id = '" . (int) $group . "'")->row;
        } else {
          $query = $this->db->query("SELECT customer_group_id, name FROM " . DB_PREFIX . "customer_group_description WHERE name = '" . $this->db->escape($group) . "'")->row;
        }
        
        if (!isset($query['customer_group_id'])) {
          $values_return[] = '['.$recurring.' customer group not found]';
        }
      }
      
      if (!empty($recurringQuery['recurring_id'])) {
        // customer group exists
        if (!empty($query['customer_group_id'])) {
          if ($this->simulation) {
            $values_return[] = '[group '.$query['name'].'] '.$recurring;
          } else {
            $values_return[] = array(
              'recurring_id' => $recurringQuery['recurring_id'],
              'customer_group_id' => $group,
            );
          }
        // customer group not exists, assign to all
        } else {
          $this->loadCustomerGroups();
          
          foreach ($this->customer_groups as $customer_group_id) {
            if ($this->simulation) {
              $values_return[] = '[group '.$customer_group_id.'] '.$recurring;
            } else {
              $values_return[] = array(
                'recurring_id' => $recurringQuery['recurring_id'],
                'customer_group_id' => $customer_group_id,
        ...
        if ($this->simulation) {
          $values_return[] = '['.$recurring.' profile not found]';
        }
      }
    }
  ...
  public function setArrayPath($line, $path, $value) {
    if (strpos($path, '*')) {
      $arrItems = explode('/', $path);
      //$arrItems = explode('/', str_replace('[0]/', '/0/', $path));
      //$initKey = array_shift($arrItems);
      ...
      foreach ($arrItems as $arrItem) {
        array_shift($arrItems);
        $currentPath .= $currentPath ? '/' . $arrItem : $arrItem;
        
        if (is_array($temp) && array_key_exists($arrItem, $temp)) {
          $temp = &$temp[$arrItem];
        } else if (is_array($temp) && $arrItem == '*') {
          foreach ($temp as $currentKey => &$temp2) {
            $arrItems2 = $arrItems;
            foreach ($arrItems2 as $arrItem2) {
              array_shift($arrItems2);
              if (is_array($temp2) && array_key_exists($arrItem2, $temp2)) {
                $temp2 = &$temp2[$arrItem2];
              }
              // arrItems is empty then set the value
              if (empty($arrItems2) && isset($value[$currentKey])) {
                $temp2 = $value[$currentKey];
              }
            }
          }
        }
        
        if (isset($line[str_replace('/*', '', $currentPath)]) && $arrItem == '*') {
          $line[str_replace('/*', '', $currentPath)] = $temp;
          
           foreach ($temp as $k => $v) {
             if (isset($line[str_replace('/*', '', $currentPath).'/'.$k])) {
               $line[str_replace('/*', '', $currentPath).'/'.$k] = $temp[$k];
             }
           }
        }
      }
