...
class ModelGkdImportProduct extends Model {
  public function __construct($registry) {
    ...
    $product_data = array('product_id', 'model', 'sku', 'upc', 'ean', 'jan', 'isbn', 'mpn', 'location', 'quantity', 'minimum', 'subtract', 'stock_status_id',
                          ...
                          'status', 'tax_class_id', 'sort_order');
    
    if (!empty($config['image_insert_type']) && empty($data['image'])) {
      // do not update main image in case image add is enabled and no image available to import
    } else {
      $product_data[] = 'image';
    }
    ...
    if (isset($data['product_discount'])) {
      ...
      if (empty($config['image_insert_type']) || (!empty($config['image_insert_type']) && $config['image_insert_type'] == 'rm_add' && !in_array($product_id, $this->session->data['obui_processed_ids']))) {
        ...
        if (!empty($config['image_insert_type'])) {
          $imageExists = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_image WHERE product_id = '" . (int)$product_id . "' AND image = '" . $this->db->escape($product_image['image']) . "'")->row;
          $mainImageExists = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "' AND image = '" . $this->db->escape($product_image['image']) . "'")->row;
          
          if (empty($imageExists) && empty($mainImageExists)) {
      ...
      if (isset($data['product_seo_url'])) { 
        foreach ($data['product_seo_url'] as $store_id => $language) {
          foreach ($language as $language_id => $keyword) {
            $this->db->query("DELETE FROM " . DB_PREFIX . "seo_url WHERE query = 'product_id=" . (int)$product_id . "' AND store_id = '" . (int)$store_id . "' AND language_id = '" . (int)$language_id . "'");
