...
class ModelCustomerCustomerApproval extends Model {
	...
	public function approveCustomer($customer_id) {
				$approver = $this->user->getUserName();
		$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET approved = '1', date_approved = NOW(), approved_by = '" . $approver ."' WHERE customer_id = '" . (int)$customer_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "customer_approval` WHERE customer_id = '" . (int)$customer_id . "' AND `type` = 'customer'");
	}

	public function denyCustomer($customer_id, $reason_denied = null ) {
		$reason_denied = str_replace("'", "\'", $reason_denied);
		$this->db->query("UPDATE `" . DB_PREFIX . "customer` SET `denied` = '1', `reason_denied` = '" . $reason_denied . "' WHERE customer_id = '" . (int)$customer_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "customer_approval` WHERE customer_id = '" . (int)$customer_id . "' AND `type` = 'customer'");
	}
