...
class ModelCatalogCategory extends Model {
      ...
      if (!empty($data['gkd_extra_fields']) && defined('GKD_UNIV_IMPORT')) {
        foreach ($data['gkd_extra_fields'] as $extra_field) {
          if (isset($data[$extra_field])) {
			...
			$this->db->query("INSERT INTO " . DB_PREFIX . "category SET " . $univimp_extra . " parent_id = '" . (int)$data['parent_id'] . "', `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), video_url = '" . $this->db->escape($data['video_url']) . "', video_name = '" . $this->db->escape($data['video_name']) . "', video_width = '" . $this->db->escape($data['video_width']) . "', video_height = '" . $this->db->escape($data['video_height']) . "' , date_added = NOW(), video_url = '" . $this->db->escape($data['video_url']) . "', video_name = '" . $this->db->escape($data['video_name']) . "', video_width = '" . $this->db->escape($data['video_width']) . "', video_height = '" . $this->db->escape($data['video_height']) . "'");
      ...
      if (!empty($data['gkd_extra_fields']) && defined('GKD_UNIV_IMPORT')) {
        foreach ($data['gkd_extra_fields'] as $extra_field) {
          if (isset($data[$extra_field])) {
			...
			$this->db->query("UPDATE " . DB_PREFIX . "category SET parent_id = '" . (int)$data['parent_id'] . "', " . $univimp_extra . " `top` = '" . (isset($data['top']) ? (int)$data['top'] : 0) . "', `column` = '" . (int)$data['column'] . "', sort_order = '" . (int)$data['sort_order'] . "', status = '" . (int)$data['status'] . "', date_modified = NOW(), video_url = '" . $this->db->escape($data['video_url']) . "', video_name = '" . $this->db->escape($data['video_name']) . "', video_width = '" . $this->db->escape($data['video_width']) . "', video_height = '" . $this->db->escape($data['video_height']) . "'  WHERE category_id = '" . (int)$category_id . "'");
	...
	public function getCategories($data = array()) {
			...
			$sql = "SELECT 
					...
					c1.sort_order, 
					(CASE 
						WHEN c1.status = '1' THEN 'Enabled' 
						ELSE 'Disabled' 
					END) AS status, 
					(SELECT count(product_id) AS product_count 
						FROM " . DB_PREFIX . "product_to_category pc 
						WHERE pc.category_id = c1.category_id) AS product_count 
