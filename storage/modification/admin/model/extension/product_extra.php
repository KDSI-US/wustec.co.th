...
class ModelExtensionProductExtra extends Model {
	...
	public function getProducts($data = array(), $language = null) {
		...
		if ($data) {
			...
			if (isset($data['filter_style_code']) && !is_null($data['filter_style_code'])) {
				$sql .= " AND LCASE(p.style_code) LIKE '%" . $this->db->escape(mb_strtolower($data['filter_style_code'])) . "%'";
			}
	...
	public function changeStyleCode($product_id, $style_code){
		if($product_id > 0){
			$query = $this->db->query("UPDATE " . DB_PREFIX . "product SET style_code='".$this->db->escape($style_code)."' WHERE product_id = '" . (int)$product_id . "'");
			$query = $this->db->query("SELECT style_code FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
		}
		$this->cache->delete('product');
		return $query->row['style_code'];
	}
	public function changeMsrp($product_id, $msrp){
		if($product_id > 0){
			$query = $this->db->query("UPDATE " . DB_PREFIX . "product SET msrp='".$this->db->escape($msrp)."' WHERE product_id = '" . (int)$product_id . "'");
			$query = $this->db->query("SELECT msrp FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
		}
		$this->cache->delete('product');
		return $query->row['msrp'];
	}
	...
	public function getMaxSpecialPrice($product_id, $customer_group_id = null){
		$where = '';
		if ($customer_group_id) {
			$where = ' AND customer_group_id = ' . (int)$customer_group_id;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id=".(int)$product_id." " . $where . " ORDER BY ps.priority DESC, ps.product_special_id LIMIT 0, 1");
		if(isset($query->row['price'])){
			return $query->row['price'];
		}
		return null;
	}
	public function getMaxDiscountPrice($product_id, $customer_group_id = null){
		$where = '';
		if ($customer_group_id) {
			$where = ' AND customer_group_id = ' . (int)$customer_group_id;
		}
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_discount pd WHERE pd.product_id=".(int)$product_id." " . $where . " ORDER BY pd.priority DESC, pd.product_discount_id LIMIT 0, 1");
		if(isset($query->row['price'])){
			return $query->row['price'];
		}
		return null;
	}
	public function changeProductSpecialPrice($product_id, $price, $customer_group_id = null){
		$where = '';
		if ($customer_group_id) {
	...
	public function changeProductDiscountPrice($product_id, $price, $customer_group_id = null){
		$where = '';
		if ($customer_group_id) {
			$group = $customer_group_id;
			$where = ' AND customer_group_id = ' . (int)$customer_group_id;
