...
class ModelExtensionModuleiSenseLabsSeo extends Model {  
    ...
    public function getSettings($store_id = null) {
        if (is_null($store_id)) {
		...
		$getSettings = $this->cache->get('isense.seosettings.' . (int)$store_id);
		...
		$results = $this->db->query("SELECT * FROM `" . DB_PREFIX . "seo_module_settings` WHERE `store_id` = '" . (int)$store_id . "'");
		...
		$this->cache->set('isense.seosettings.' . (int)$store_id, $response);
    ...
    public function saveSetting($key, $value, $store_id = null) {
		...
		$this->cache->delete('isense.seosettings');
		$this->getSettings($store_id);
    ...
    public function SeoProductH1Tags($product_ids = array(), $language_id, $store_id = '0') {
        ...
        $search     = array('[product]', '[model]', '[sku]', '[upc]', '[random]', '[lang]', '[manufacturer]', '[id]', '[site_name]');
        ...
        foreach ($product_ids as $id) {
            ...
            $replace = array($product_info['name'], $product_info['model'], $product_info['sku'], $product_info['upc'], rand(), $language_code, $product_info['manufacturer'], $product_info['product_id'], $site_name);
    ...
    private function SeoProductTitles($product_ids = array(), $language_id) {
        ...
        $patternDecode = json_decode($pattern, true);
		if (is_array($patternDecode)) {
			$pattern = $patternDecode;
		}
    ...
    public function formatSEOUrl($string, $useCyrillicUrl = false) {
        ...
        if($useCyrillicUrl){
            ...
            if (function_exists('mb_substr')) {
                ...
                $result = trim(mb_substr($result, 0, 200, 'utf-8'));

            } else {

                $result = trim(substr($result, 0, 200, 'utf-8'));

            }

            $result = htmlspecialchars($this->sanitizeChar($result), ENT_QUOTES, 'UTF-8');
            $result = str_replace('-html','.html', $result);
