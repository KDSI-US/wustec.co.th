...
class ModelExtensionModuleSolookbook extends Model {
	...
	function addLookBookSlider($data = array()) {
		...
		$this->db->query("INSERT INTO `" . DB_PREFIX . "module` SET `name` = '" . $this->db->escape($module_data['name']) . "', `code` = 'so_lookbook_slider', `setting` = '" . $this->db->escape(json_encode($module_data)) . "'");
	...
	public function getProductBySku($term) {
		$sql = "SELECT p.product_id, p.sku FROM " . DB_PREFIX . "product p WHERE p.sku LIKE '" . $term . "%' AND p.status = 1";
	...
	public function getProductBySku2($sku) {
