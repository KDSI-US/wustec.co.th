	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_setting_module->addModule('mpbloglatest', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = [
				...
				'href' => $this->url->link('extension/module/mpbloglatest', $this->token.'=' . $this->session->data[$this->token], true)
			];
		} else {
			$data['breadcrumbs'][] = [
				'text' => strip_tags($this->language->get('heading_title')),
				'href' => $this->url->link('extension/module/mpbloglatest', $this->token.'=' . $this->session->data[$this->token] . '&module_id=' . $this->request->get['module_id'], true)
			];
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/mpbloglatest', $this->token.'=' . $this->session->data[$this->token], true);
		} else {
			$data['action'] = $this->url->link('extension/module/mpbloglatest', $this->token.'=' . $this->session->data[$this->token] . '&module_id=' . $this->request->get['module_id'], true);
		}
		...
		$this->response->setOutput($this->viewLoad('extension/module/mpbloglatest', $data));
	}

	protected function validate() {
		if (!$this->user->hasPermission('modify', 'extension/module/mpbloglatest')) {
