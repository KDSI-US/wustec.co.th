...
class ControllerExtensionModuleSoMegaMenu extends Controller {
    ...
    public function index() {
        ...
        if(isset($_GET['edit'])) {
			...
			$dane['content']['subcategory']['category'] = (isset($dane['content']['subcategory']['category']) && $dane['content']['subcategory']['category']) ? @json_decode($dane['content']['subcategory']['category'], true) :  array();
			...
			if($dane) {
                ...
                $data['link'] 		= (isset($dane['link']) && $dane['link']) ? @json_decode($dane['link'], true) :  array();
		...
		if(isset($this->request->get['module_id'])){
		    $data['linkexport'] =  $this->url->link('extension/module/so_megamenu/export_data&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id']);

			$data['linkimport'] =  $this->url->link('extension/module/so_megamenu/linkimport&user_token=' . $this->session->data['user_token'].'&module_id='.$this->request->get['module_id']);			
		}
        // Remove cache
    ...
    public function export_data() {
		...
		$this->load->model('extension/module/so_megamenu');
		$query = $this->model_extension_module_so_megamenu->getAllMenu($this->request->get['module_id']);
		
		foreach ($query as $key => $result) {
			$query[$key]['module_id'] = '';
		}
		
		$name = strftime('megamenu_%m_%d_%Y.json');
	...
	public function linkimport() {
		...
		{			
			...
			foreach ($query as $key => $result) {
				$this->model_extension_module_so_megamenu->deleteAllMenu($result['id']);
			}
			
			$querys = $this->model_extension_module_so_megamenu->getAllMenu($this->request->get['module_id']);
			if(count($querys->rows) == 0) {
				foreach ($r as $key => $data) {
					$f=(array)$data;
					$f['display_submenu'] = $f['submenu_type'];
					$this->model_extension_module_so_megamenu->addMenu($f);
				}				
			}
