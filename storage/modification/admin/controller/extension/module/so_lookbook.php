...
require_once (DIR_SYSTEM.'library/so_lookbook/Uploadedfilexhr.php');
require_once (DIR_SYSTEM.'library/so_lookbook/Uploadedfileform.php');

class ControllerExtensionModuleSoLookBook extends Controller {
	protected $uploader;
	protected $uploadXhr;
	...
	public function index() {
		$this->load->language('extension/module/so_lookbook');
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$lookbook_id = $this->model_extension_module_so_lookbook->addLookBook($this->request->post);
			...
			if(isset($this->request->post['save_stay']) and $this->request->post['save_stay']=1) {
				$this->response->redirect($this->url->link('extension/module/so_lookbook/edit', 'user_token=' . $this->session->data['user_token'] . '&lookbook_id='.$lookbook_id, true));
			}
			else {
				$this->response->redirect($this->url->link('extension/module/so_lookbook', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
		}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->request->post['pins'] = htmlspecialchars_decode($this->request->post['pins'], ENT_COMPAT);
			$this->model_extension_module_so_lookbook->editLookBook($this->request->get['lookbook_id'], $this->request->post);
			...
			if(isset($this->request->post['save_stay']) and $this->request->post['save_stay']=1) {
				$this->response->redirect($this->url->link('extension/module/so_lookbook/edit', 'user_token=' . $this->session->data['user_token'] . '&lookbook_id='.$this->request->get['lookbook_id'], true));
			}
			else {
				$this->response->redirect($this->url->link('extension/module/so_lookbook', 'user_token=' . $this->session->data['user_token'] . $url, true));
			}
	...
	protected function getList() {
		...
		$data['add'] = $this->url->link('extension/module/so_lookbook/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		$data['delete'] = $this->url->link('extension/module/so_lookbook/delete', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$lookbook_total = $this->model_extension_module_so_lookbook->getTotalLookBook($filter_data);

		$results = $this->model_extension_module_so_lookbook->getLookBook($filter_data);
		...
		foreach ($results as $result) {
			...
			$data['lookbooks'][] = array(
				...
				'edit'	=> $this->url->link('extension/module/so_lookbook/edit', 'user_token=' . $this->session->data['user_token'] . '&lookbook_id=' . $result['lookbook_id'] . $url, true)
			);
		}
		...
		$pagination->url = $this->url->link('extension/module/so_lookbook', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);
		...
		$this->response->setOutput($this->load->view('extension/module/so_lookbook/lookbook_list', $data));
	}
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $lookbook_id) {
				$this->model_extension_module_so_lookbook->deleteLookBook($lookbook_id);
			}
	...
	protected function getForm() {
		$this->document->addScript('view/javascript/so_lookbook/js/fileuploader.js');
		$this->document->addScript('view/javascript/so_lookbook/js/jquery-ui.js');
		$this->document->addScript('view/javascript/so_lookbook/js/jquery.annotate.js');
		$this->document->addScript('view/javascript/so_lookbook/js/json2.min.js');		
        $this->document->addStyle('view/javascript/so_lookbook/css/lookbook.css');
        $this->document->addStyle('view/javascript/so_lookbook/css/styles.css');
		...
		if (!isset($this->request->get['lookbook_id'])) {
			$data['action'] = $this->url->link('extension/module/so_lookbook/add', 'user_token=' . $this->session->data['user_token'] . $url, true);
		} else {
			$data['action'] = $this->url->link('extension/module/so_lookbook/edit', 'user_token=' . $this->session->data['user_token'] . '&lookbook_id=' . $this->request->get['lookbook_id'] . $url, true);
		}

		$data['cancel'] = $this->url->link('extension/module/so_lookbook', 'user_token=' . $this->session->data['user_token'] . $url, true);

		$data['action_upload'] = str_replace('&amp;', '&', $this->url->link('extension/module/so_lookbook/upload', 'user_token=' . $this->session->data['user_token'], true));

		$data['check_product_url'] = str_replace('&amp;', '&', $this->url->link('extension/module/so_lookbook/checkproduct', 'user_token=' . $this->session->data['user_token'], true));
		$data['load_product_url'] = str_replace('&amp;', '&', $this->url->link('extension/module/so_lookbook/loadproduct', 'user_token=' . $this->session->data['user_token'], true));

		if (isset($this->request->get['lookbook_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$lookbook_info = $this->model_extension_module_so_lookbook->getLookBookInfo($this->request->get['lookbook_id']);
		}
		...
		$data['radius'] = round($data['pin_width']/2);

		$data['header'] = $this->load->controller('common/header');
		$data['column_left'] = $this->load->controller('common/column_left');
		$data['footer'] = $this->load->controller('common/footer');

		$this->response->setOutput($this->load->view('extension/module/so_lookbook/lookbook_form', $data));
	}

	protected function validateForm() {
		if (!$this->user->hasPermission('modify', 'extension/module/so_lookbook')) {
    ...
    public function checkproduct() {
    	$product_id = 0;
    	$sku = $this->request->post['text'];
    	$defaultPinText = '+';
    	$labelPost = $this->request->post['label'];
    	if($sku!=''){
    		$this->load->model('extension/module/so_lookbook');
    		$product = $this->model_extension_module_so_lookbook->getProductBySku2($sku);
    ...
    public function loadproduct() {
    	...
    	$responseData = array();
    	$results = $this->model_extension_module_so_lookbook->getProductBySku($q);
	...
	function install() {
		$this->load->model('extension/module/so_lookbook');
		$this->model_extension_module_so_lookbook->install();
	}

	function uninstall() {
		$this->load->model('extension/module/so_lookbook');
		$this->model_extension_module_so_lookbook->uninstall();
		$this->load->model('setting/setting');
