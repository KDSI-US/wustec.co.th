...
class ControllerExtensionModuleSoPageBuilder extends Controller
{
	...
	public function index()
	{
		...
		if (isset($this->request->get['module_id'])) {
			$data['linkexport'] =  $this->url->link('extension/module/so_page_builder/export_data&user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id']);

			$data['linkimport'] =  $this->url->link('extension/module/so_page_builder/linkimport&user_token=' . $this->session->data['user_token'] . '&module_id=' . $this->request->get['module_id']);
		}
	...
	public function export_data()
	{
		$this->load->model('extension/module/so_page_builder');
		if ($this->_checkPermission()) {
			$query = $this->model_extension_module_so_page_builder->exportModule($this->request->get['module_id']);
			$query->row['module_id'] = '';
			$name = strftime('page_builder_%m_%d_%Y.json');
	...
	public function linkimport()
	{
		...
		if (isset($r) || $r != null || isset($r->row->name)) {
			$this->model_extension_module_so_page_builder->updateModule($this->request->get['module_id']);
			$query = $this->model_setting_module->getModule($this->request->get['module_id']);
			if ($query == "") {
				$this->model_extension_module_so_page_builder->importModule($this->request->get['module_id'], (array)$r->row);
			}

			$this->response->redirect($this->url->link('extension/module/so_page_builder', 'module_id=' . $this->request->get['module_id'] . '&user_token=' . $this->session->data['user_token'], 'SSL'));
		} else {
