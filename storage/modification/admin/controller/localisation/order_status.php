...
class ControllerLocalisationOrderStatus extends Controller {
    ...
    $data['orderstatus_colors_status'] = $this->config->get('module_como_orderstatus_status') && $this->config->get('module_como_orderstatus_colors_status');
    if ($data['orderstatus_colors_status']) {
    ...
    'name' => ($data['orderstatus_colors_status'] ? $this->model_extension_module_como_orderstatus->getOrderStatusName($result) : $result['name']) . (($result['order_status_id'] == $this->config->get('config_order_status_id')) ? $this->language->get('text_default') : null),
    ...
    if ($this->config->get('module_como_orderstatus_status') && $this->config->get('module_como_orderstatus_colors_status')) {
    ...
    $data['orderstatus_colors_status'] = $this->config->get('module_como_orderstatus_status') && $this->config->get('module_como_orderstatus_colors_status');
    if ($data['orderstatus_colors_status']) {
        $order_status_colors = false;
        if (isset($this->request->get['order_status_id']) && $this->request->get['order_status_id']) {
            $this->load->model('extension/module/como_orderstatus');
            $order_status_colors = $this->model_extension_module_como_orderstatus->getOrderStatusColors($this->request->get['order_status_id']);
        }
        if (!$order_status_colors) {
            $order_status_colors = array('color' => '', 'bgcolor' => '', 'lbcolor' => '');
        }
        if (isset($this->request->post['color'])) {
            $data['color'] = $this->request->post['color'];
        } else {
            $data['color'] = $order_status_colors['color'];
        }
        if (isset($this->request->post['bgcolor'])) {
            $data['bgcolor'] = $this->request->post['bgcolor'];
        } else {
            $data['bgcolor'] = $order_status_colors['bgcolor'];
        }
        if (isset($this->request->post['lbcolor'])) {
            $data['lbcolor'] = $this->request->post['lbcolor'];
        } else {
            $data['lbcolor'] = $order_status_colors['lbcolor'];
        }
    }
