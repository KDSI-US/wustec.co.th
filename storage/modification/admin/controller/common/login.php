...
class ControllerCommonLogin extends Controller {
	...
	public function index() {
		...
		$config_secure_admin_url = $this->config->get('config_secure_admin_url');
		$admin_url = null;
		$admin_url_params = trim($config_secure_admin_url['admin_url_params'] ? : "");
		$admin_url_key =  trim($config_secure_admin_url['admin_url_key'] ? : "");
		$admin_url = empty($admin_url_params) ? "" : ($admin_url = "&" . $admin_url_params . "=" . $admin_url_key );
		if (!empty($admin_url_params) && !empty($admin_url_key))  {
			if ($this->request->get[$admin_url_params] != $admin_url_key) {
				$this->response->redirect("//".$_SERVER['HTTP_HOST']."/");
			}
		}
		...
		if (!empty($config_secure_admin_url['admin_url_params']) && !empty($config_secure_admin_url['admin_url_key'])) {
		...
		$store_id = '0';
		$data['config_owner'] = $this->model_setting_setting->getSettingValue('config_owner', $store_id);
		...
		$this->response->setOutput($this->load->view('extension/extension/login', $data));
