...
class ControllerCheckoutGuest extends Controller {
	public function index() {
		...
		$gueststatus=$this->config->get('registermanager_guestregister');

		if(!empty($modulestatus) && !empty($gueststatus)) {
			...
			$data['phonerequiredstatus'] 	 = $this->config->get('registermanager_phonerequired');$data['add1requiredstatus'] 	= $this->config->get('registermanager_add1required');
		...
		if(!empty($modulestatus) && !empty($gueststatus)) {
			$this->response->setOutput($this->load->view('checkout/tmdguest', $data));
		} else {
