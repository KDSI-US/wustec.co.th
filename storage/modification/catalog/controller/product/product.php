...
class ControllerProductProduct extends Controller {
	...
	public function index() {
            ...
            if ($login_required['status_product']) {
                $redirect = true;
                ...
                if ($redirect) {
                    ...
                    $this->session->data['redirect'] = $this->url->link('product/product', 'path=' . $path .'&product_id=' . $product_id, true);
		...
		if ($approval_required['status_product']) {
			$redirect = true;
			if (isset($this->request->get['route'])) {
				$route = $this->request->get['route'];
			} else {
		...
		if ($product_info) {
            ...
            $data['sweetalert_fragment_in_product'] = $this->load->controller('extension/module/sweetalert/fragment_in_product');
        ...
        if (!isset($category_info)) {
          ...
          if ($categories) {
			...
			$data['PRO_settings'] = $this->config->get('module_related_options_settings');
			$data['PRO_status'] = $this->config->get('module_related_options_status');
			
$this->document->addScript('catalog/view/javascript/JsBarcode.all.js');
		...
		$this->document->addScript('catalog/view/javascript/jquery/swiper/js/swiper.jquery.js');
			...
			if(isset($this->request->get['product_id']) && $this->config->get('mpplan_status') && $this->config->get('mpplan_product_id') == $this->request->get['product_id']) {
				$this->response->redirect($this->url->link('membership/prices', '', true));
			}

			if($this->config->get('mpplan_status') && $this->config->get('mpplan_product_status')) {
				$this->load->language('membership/information');
				$this->load->model('membership/prices');

				$mpplan_description = (array)$this->config->get('mpplan_description');
				$data['membership_buttontext'] = !empty($mpplan_description[$this->config->get('config_language_id')]['buttontext']) ? $mpplan_description[$this->config->get('config_language_id')]['buttontext'] : $this->language->get('membership_buttontext');

				$data['membership_modaltext'] = $this->language->get('membership_modaltext');
				$data['column_package'] = $this->language->get('column_package');
				$data['column_regular'] = $this->language->get('column_regular');
				$data['column_membership'] = $this->language->get('column_membership');
				$data['column_yousave'] = $this->language->get('column_yousave');

				$data['membership_prices'] = array();
				if($this->customer->isLogged()) {
				...
				$membership_prices = $this->model_membership_prices->getProductPlans($product_info['product_id']);
				foreach ($membership_prices as $membership_price) {
					$mpplan_info = $this->model_membership_prices->getMpplan($membership_price['mpplan_id']);
					if($mpplan_info) {
						$plan_discount = $this->model_membership_prices->getMpplanDiscount($mpplan_info['mpplan_id'], $customer_group_id);

						$regular_price = false;
						if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
							$regular_price = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'));
						}
						if ((float)$product_info['special']) {
							$regular_price = $this->tax->calculate($product_info['special'], $product_info['tax_class_id'], $this->config->get('config_tax'));
						}

						if($regular_price && $plan_discount['discount']) {
							$membership_price = $regular_price - (($regular_price * $plan_discount['discount']) / 100);
						} else {
							$membership_price = $regular_price;
						}

						$save_price = $regular_price - $membership_price;

						$data['membership_prices'][] = array(
							'plan_name'			=> $mpplan_info['name'],
							'price'       		=> $this->currency->format($regular_price, $this->session->data['currency']),
		...
		$data['sku'] = $product_info['sku'];
			...
			if ($this->customer->isLogged() || !$this->config->get('config_customer_price')) {
				...
				$data['price_no_sign'] = $this->tax->calculate($product_info['price'], $product_info['tax_class_id'], $this->config->get('config_tax'));
			} else {
				$data['price'] = false;

				$data['price_no_sign'] = false;
			}
			...
			$min_price = $this->model_catalog_product->getMinPriceForVolumeDiscountTable($this->request->get['product_id']);
			$data['min_price'] = $this->currency->format($this->tax->calculate((float) $product_info['price'] + (float) $min_price, $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']);

			foreach ($discounts as $discount) {
				$data['discounts'][] = array(
					'quantity' => $discount['quantity'],
					'price'    => $this->currency->format($this->tax->calculate($discount['price'], $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency']),
					'min_price'    => $this->currency->format($this->tax->calculate((float) $discount['price'] + (float) $min_price, $product_info['tax_class_id'], $this->config->get('config_tax')), $this->session->data['currency'])
				);
			}
				...
				$option_value['subtract'] = false;
					...
					if (!$option_value['subtract'] || ($option_value['quantity'] > 0)) {
						if ((($this->config->get('config_customer_price') && $this->customer->isLogged()) || !$this->config->get('config_customer_price')) && (float)$option_value['price']) {
							...
							$price_no_sign = $this->tax->calculate($option_value['price'], $product_info['tax_class_id'], $this->config->get('config_tax') ? 'P' : false);
				...
				$product_option_value_images[$option_value['product_option_value_id']] = array(
					...
					'image_large'    => $option_image_popup
						...
						if ($option_value['quantity'] <= 0) {
							$option_stock= $product_info['stock_status'];
						} elseif ($this->config->get('config_stock_display')) {
							$option_stock = $option_value['quantity'];
						} else {
							$option_stock = $this->language->get('text_instock');
						}
						$product_option_value_data[] = array(
							...
							'price'                   => $price,
							'price_no_sign'						=> $price_no_sign,
							'option_image'	=> $this->model_tool_image->resize($option_value['option_image'], 1280, 1920),
			...
			$data['PRO_option_images'] = isset($product_option_value_images) ? json_encode($product_option_value_images, $json_flags) : null;
			...
			foreach ($results as $result) {
				...
				$data['products'][] = array(
					...
					'thumb_swap'  	=> $this->model_tool_image->resize($images, $this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_width'),
						$this->config->get('theme_' . $this->config->get('config_theme') . '_image_related_height')),
'href' => $this->url->link_for_related('product/product', $url . '&product_id=' . $result['product_id']),
			
				);
			}


		$data['related_count'] = count($data['products']);
			

      $data['mgallery_products'] = '';
      if ($this->config->get('gallery_setting_status')) {
        $data['mgallery_products'] = $this->load->controller('extension/mpphotogallery/product');
      }
			...
			$this->model_catalog_product->updateViewed($this->request->get['product_id']);
                ...
                $filter_galleria = array(
                    'source' => 'product',
                    'source_id'  => $this->request->get['product_id']
                );
                $data['galleria'] = $this->load->controller('extension/module/galleria/widget', $filter_galleria);
            

		$this->document->addStyle('catalog/view/javascript/so_lookbook/css/styles.css');
		...
		$lookbook = $this->model_extension_module_so_lookbook->getLookBook($product_info['lookbook_id']);
		if (!empty($lookbook)) {
			$data['lookbook_image'] = $base_url.'image/'.$lookbook['image'];
			$data['lookbook_name'] = $lookbook['name'];
			$data['lookbook'] = $lookbook;
			$data['pinHtml'] = '';
			if ($data['lookbook']) {
				$data['pinHtml'] = $this->model_extension_module_so_lookbook->getPinHtml($lookbook['lookbook_id']);
			}
		}
			...
			$data['PRO_config'] = json_encode(array(
				'price_animate_on'    => (int)$data['PRO_settings']['price_animate_on'],
				'price_multiply'      => (int)$data['PRO_settings']['price_multiply'],
				'option_selector'     => isset($data['PRO_settings']['option_selector'])    ? html_entity_decode($data['PRO_settings']['option_selector'], ENT_QUOTES, 'UTF-8')    : '',
				'price_selector'      => isset($data['PRO_settings']['price_selector'])     ? html_entity_decode($data['PRO_settings']['price_selector'], ENT_QUOTES, 'UTF-8')     : '',
				'price_old_selector'  => isset($data['PRO_settings']['price_old_selector']) ? html_entity_decode($data['PRO_settings']['price_old_selector'], ENT_QUOTES, 'UTF-8') : '',
				'quantity_selector'   => isset($data['PRO_settings']['quantity_selector'])  ? html_entity_decode($data['PRO_settings']['quantity_selector'], ENT_QUOTES, 'UTF-8')  : ''
			), $json_flags);

			$data['PRO_product_data'] = json_encode(array(
			...
			$data['PRO_options_data'] = json_encode($options_data, $json_flags);
			...
			$data['product_block'] = $this->load->controller('common/product_block');

			$other_fit_product_id = $this->model_catalog_product->getOtherFitProduct($this->request->get['product_id']);
			if ($other_fit_product_id) {
				$data['other_fit_product_href'] = $this->url->link('product/product','product_id=' . $other_fit_product_id, true);
			} else {
				$data['other_fit_product_href'] = false;
			}
