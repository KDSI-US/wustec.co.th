...
class ControllerExtensionModulempblogPopular extends Controller {
	public function index($setting) {
		if ($this->config->get('mpblog_status')) {
			...
			$results = $this->model_extension_mpblog_mpblogpost->getPopularMpBlogPosts($setting['limit']);
			...
			if ($results) {
				...
				if ($data['position'] == 'column_left' || $data['position'] == 'column_right') {
					return $this->load->view('extension/module/mpblogpopular_list', $data);
				} else {
					return $this->load->view('extension/module/mpblogpopular_grid', $data);
				}
			}
		}
	}
