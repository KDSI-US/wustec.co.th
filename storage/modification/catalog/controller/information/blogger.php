...
class ControllerInformationBlogger extends Controller
{
	...
	public function index()
	{
		...
		$language = $this->model_extension_module_blogger->getLanguageByCode($this->session->data['language']);

		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_extension_module_blogger->addComment($this->request->get['blogger_id'], $this->request->post, $language['language_id']);
		...
		if ($blog_info) {
			...
			$module = $this->model_extension_module_blogger->getModule($blog_info['module_id']);
			...
			$blog_comments = $this->model_extension_module_blogger->getBlogComments($this->request->get['blogger_id'], $language['language_id']);
	...
	public function blogs()
	{
		...
		$blogs = $this->model_extension_module_blogger->getBlogs();

		foreach ($blogs as $blog) {
			$module_blog_data = array();
			$module = $this->model_extension_module_blogger->getModule($blog['module_id']);

			$setting = json_decode($module['setting'], true);

			if (!empty($setting['status']) && $setting['status']) {
				$module_blogs = $this->model_extension_module_blogger->getBlogsByModule($blog['module_id']);

				foreach ($module_blogs as $module_blog) {

					$total_comments = $this->model_extension_module_blogger->getTotalBlogComments($module_blog['blogger_id']);
					$module_blog_data[] = array(
