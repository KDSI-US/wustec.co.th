...
class ModelCheckoutOrder extends Model {
	...
	public function addOrderShipment($order_id, $shipment_date_added, $shipping_courier_id, $tracking_number) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "order_shipment SET order_id = '" . (int)$order_id . "', date_added = '" . $shipment_date_added . "', shipping_courier_id = '" . (int)$shipping_courier_id . "', tracking_number = '" . $this->db->escape($tracking_number) . "'");
	}	

	public function deleteOrderShipment($order_id, $order_shipment_id) {
		$this->db->query("DELETE FROM " . DB_PREFIX . "order_shipment WHERE order_shipment_id = '" . (int)$order_shipment_id . "' AND order_id = '" . (int)$order_id . "'");
	}	
	...
	public function addOrder($data) {
		...
		if (is_file($file)) {
			if ($this->config->get('config_checkout_id')) {
				...
				$this->gdpr->newAcceptanceRequest($this->config->get('config_checkout_id'), $data['email'], false);
			}
		}
		...
		if (isset($data['products'])) {
			foreach ($data['products'] as $product) {
				...
				if(!empty($product['mpplan_details']) && $this->config->get('mpplan_status')) {
					$this->db->query("INSERT INTO `" . DB_PREFIX . "mpplan_customer` SET `order_id` = '" . (int)$order_id . "', `order_product_id` = '" . (int)$order_product_id . "', `customer_id` = '" . (int)$data['customer_id'] . "', `mpplan_id` = '" . (int)$product['mpplan_details']['mpplan_id'] . "', `plan_name` = '" . $this->db->escape($product['mpplan_details']['plan_name']) . "', `start_date` = '" . $this->db->escape($product['mpplan_details']['start_date']) . "', `end_date` = '" . $this->db->escape($product['mpplan_details']['end_date']) . "', `duration_type` = '" . $this->db->escape($product['mpplan_details']['duration_type']) . "', `duration_value` = '" . $this->db->escape($product['mpplan_details']['duration_value']) . "', `plan_info` = '" . $this->db->escape($product['mpplan_details']['plan_info']) . "'");
				}
				/* MP Membership Ends */
		...
		/* Comment for Cargo Shipping */
		if (isset($data['shipping_cargo_comment']) && ($data['shipping_method'] == 'Cargo Shipping')) {
			$check_exist_query = $this->db->query("SELECT * FROM `" . DB_PREFIX . "order_shipment_comment` WHERE order_id = '" . (int)$order_id . "'");
			if ($check_exist_query->num_rows) {
				$this->db->query("UPDATE " . DB_PREFIX . "order_shipment_comment SET shipment_comment = '" . $this->db->escape($data['shipping_cargo_comment']) . "', date_added = NOW() WHERE order_id = '" . (int)$order_id . "'");
			} else {
				$this->db->query("INSERT INTO " . DB_PREFIX . "order_shipment_comment SET order_id = '" . (int)$order_id . "', shipment_comment = '" . $this->db->escape($data['shipping_cargo_comment']) . "', date_added = NOW()");
			}
		}
	...
	public function editOrder($order_id, $data) {
		...
		if($this->config->get('mpplan_status')) {
			$this->db->query("DELETE FROM `" . DB_PREFIX . "mpplan_customer` WHERE order_id = '" . (int)$order_id . "'");
		}
		/* MP Membership Ends */
	...
	public function deleteOrder($order_id) {
		...
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_option` WHERE order_id = '" . (int)$order_id . "'");
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_voucher` WHERE order_id = '" . (int)$order_id . "'");
$this->db->query("DELETE FROM `" . DB_PREFIX . "order_shipment_comment` WHERE order_id = '" . (int)$order_id . "'"); /* Added for Delete Order */
		$this->db->query("DELETE FROM `" . DB_PREFIX . "order_total` WHERE order_id = '" . (int)$order_id . "'");
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		/* MP Membership Starts */
		if($this->config->get('mpplan_status')) {
		if($order_status_id == $this->config->get('mpplan_status_id')) {
			$mpplan_query = $this->db->query("SELECT * FROM ". DB_PREFIX ."mpplan_customer WHERE order_id = '". (int)$order_id ."' ORDER BY mpplan_customer_id DESC LIMIT 0,1");
			if($mpplan_query->row) {
				$this->db->query("UPDATE ". DB_PREFIX ."mpplan_customer SET active = '0' WHERE customer_id = '". (int)$mpplan_query->row['customer_id'] ."'");

				$this->db->query("UPDATE ". DB_PREFIX ."mpplan_customer SET active = '1' WHERE mpplan_customer_id = '". (int)$mpplan_query->row['mpplan_customer_id'] ."' AND customer_id = '". (int)$mpplan_query->row['customer_id'] ."'");
