...
class ModelCatalogProduct extends Model {
	...
	public function getProductsAllData($data = array(), $customer) 
	{
		...
		{
			$customer_group_id = $config_customer_group_id;
		}
				...
				*, 
				...
				(SELECT 
					...
					WHERE 
						...
						AND ps.customer_group_id = '" . (int)$config_customer_group_id . "' 
				...
				(SELECT 
						points 
					FROM `" . DB_PREFIX . "product_reward` AS pr 
					WHERE 
				...
				(SELECT 
						ss.name 
					FROM `" . DB_PREFIX . "stock_status` AS ss 
					WHERE 
						ss.stock_status_id = p.stock_status_id 
						AND ss.language_id = '" . (int)$config_language_id . "'
				) AS stock_status, 
				(SELECT 
						wcd.unit 
					...
					WHERE 
						...
						AND wcd.language_id = '" . (int)$config_language_id . "'
				) AS weight_class, 
				(SELECT 
						lcd.unit 
					FROM `" . DB_PREFIX . "length_class_description` AS lcd 
					WHERE 
						p.length_class_id = lcd.length_class_id 
						AND lcd.language_id = '" . (int)$config_language_id . "'
				) AS length_class, 
				(SELECT 
						AVG(rating) AS total 
					FROM `" . DB_PREFIX . "review` AS r1 
					WHERE 
		...
		if (!empty($data['filter_category_id'])) 
		{
			if (!empty($data['filter_sub_category'])) 
			{
				$sql .= "
					...
					LEFT JOIN `" . DB_PREFIX . "product_to_category` AS p2c 
						ON (cp.category_id = p2c.category_id) ";
			} 
			else 
			{
				$sql .= "FROM `" . DB_PREFIX . "product_to_category` AS p2c ";
			}
			if (!empty($data['filter_filter'])) 
			{
				$sql .= " 
					LEFT JOIN `" . DB_PREFIX . "product_filter` AS pf 
		...
		$sql .= "
			...
			LEFT JOIN `" . DB_PREFIX . "product_to_store` AS p2s 
				ON (p.product_id = p2s.product_id) 
			LEFT JOIN `" . DB_PREFIX . "manufacturer` AS m 
				ON (p.manufacturer_id = m.manufacturer_id) 
			WHERE 
				pd.language_id = '" . (int)$config_language_id . "' 
				AND p.status = '1' 
				AND p.date_available <= NOW() 
				AND p2s.store_id = '" . (int)$config_store_id . "' ";
		...
		if ($query->num_rows) 
		{
			...
			if(count($specialIds) > 0) 
			{
				$strSql = "SELECT 
						...
						product_id 
					FROM `" . DB_PREFIX . "product_special` AS ps 
					WHERE ps.product_special_id IN (" . implode(',', $specialIds) . ") ";
				$specialsQuery = $this->db->query($strSql);
	...
	public function getProductSeoUrls($product_id) 
	{
		...
		$strSql = "SELECT * 
			...
			WHERE 
				...
				AND language_id = '" . (int)$config_language_id . "' 
				AND store_id = '" . (int)$config_store_id . "' ";
		$query = $this->db->query($strSql);
		if ($query->num_rows) 
		{
	...
	public function getProductBySeoUrl($seoUrl) 
	{
		...
		$strSql = "SELECT * 
			...
			WHERE 
				...
				AND `keyword` = '" . $this->db->escape($seoUrl) . "' 
				AND `language_id` = '" . (int)$config_language_id . "' 
				AND `store_id` = '" . (int)$config_store_id . "' ";
		$query = $this->db->query($strSql);
		if ($query->num_rows) 
	...
	public function getProductsTotal($data = array(), $customer, $onlycount=true) 
	{
		...
		$sql .= "
			...
			WHERE 
				p.status = '1' 
				AND p.date_available <= NOW() 
				AND pd.language_id = '" . (int)$config_language_id . "' 
				AND p2s.store_id = '" . (int)$config_store_id . "' ";
	...
	private function filterSql($data, $onlycount = false) 
	{
		...
		if (!empty($data['filter_date_added_to']) && !empty($data['filter_date_added_from'])) 
		{
			$sql .= " 
				AND p.date_added 
					BETWEEN STR_TO_DATE('" . $this->db->escape($data['filter_date_added_from']) . "','%Y-%m-%d %H:%i:%s') 
					AND STR_TO_DATE('" . $this->db->escape($data['filter_date_added_to']) . "','%Y-%m-%d %H:%i:%s') ";
		} 
		...
		if (!empty($data['filter_date_modified_to']) && !empty($data['filter_date_modified_from'])) 
		{
			$sql .= " 
				AND p.date_modified 
					BETWEEN STR_TO_DATE('" . $this->db->escape($data['filter_date_modified_from']) . "','%Y-%m-%d %H:%i:%s') 
					AND STR_TO_DATE('" . $this->db->escape($data['filter_date_modified_to']) . "','%Y-%m-%d %H:%i:%s') ";
		} 
	...
	public function getProductsData($data = array(), $customer) 
	{
		...
		if (!empty($data['filter_category_id'])) 
		{
			if (!empty($data['filter_sub_category'])) 
			{
				$sql .= " FROM `" . DB_PREFIX . "category_path` AS cp 
					LEFT JOIN `" . DB_PREFIX . "product_to_category` AS p2c 
					ON (cp.category_id = p2c.category_id) ";
			} 
			else 
			...
			if (!empty($data['filter_filter'])) 
			{
				$sql .= " LEFT JOIN `" . DB_PREFIX . "product_filter` AS pf 
					ON (p2c.product_id = pf.product_id) 
					LEFT JOIN `" . DB_PREFIX . "product` AS p 
					ON (pf.product_id = p.product_id) ";
			} 
		...
		$strSql = "SELECT 
				...
				(SELECT product_special_id 
					...
					WHERE 
						...
						AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) 
						AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) 
					ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS special_id, 
				(SELECT points 
					FROM `" . DB_PREFIX . "product_reward` AS pr 
					WHERE 
						pr.product_id = p.product_id 
				...
				(SELECT ss.name 
					...
					WHERE 
						ss.stock_status_id = p.stock_status_id 
						AND ss.language_id = '" . (int)$config_language_id . "') AS stock_status, 
				(SELECT wcd.unit 
					FROM `" . DB_PREFIX . "weight_class_description` AS wcd 
					WHERE 
						p.weight_class_id = wcd.weight_class_id 
						AND wcd.language_id = '" . (int)$config_language_id . "') AS weight_class, 
				(SELECT lcd.unit 
					FROM `" . DB_PREFIX . "length_class_description` AS lcd 
					WHERE 
						p.length_class_id = lcd.length_class_id 
						AND lcd.language_id = '" . (int)$config_language_id . "') AS length_class, 
	...
	public function getOrderStatusByName($status) 
	{
		...
		$strSql = "SELECT 
			...
			WHERE 
				LCASE(name) = '" . $this->db->escape(utf8_strtolower($status)) . "' 
				AND language_id = '" . (int)$config_language_id . "' ";
		$query = $this->db->query($strSql);
		return $query->rows;
	}
	...
	public function getWeightClasses($data = array()) 
	{
		...
		if ($data) {
			$sql = "SELECT * 
				FROM `" . DB_PREFIX . "weight_class` AS wc 
				LEFT JOIN `" . DB_PREFIX . "weight_class_description` AS wcd 
				ON (wc.weight_class_id = wcd.weight_class_id) 
				WHERE wcd.language_id = '" . (int)$config_language_id . "' ";
			$sort_data = array(
				'title',
				'unit',
				'value'
		...
		{
			$weight_class_data = $this->cache->get('weight_class.' . (int)$config_language_id);
			if (!$weight_class_data) 
			{
				...
				$this->cache->set('weight_class.' . (int)$config_language_id, $weight_class_data);
			}
	...
	public function getStockStatuses($data = array())
	{
		...
		if ($data) 
		{
			$sql = "SELECT ss.* 
				FROM `" . DB_PREFIX . "stock_status` AS ss 
				WHERE ss.language_id = '" . (int)$config_language_id . "' 
				ORDER BY name ";
			if (isset($data['order'])) $sql .= $data['order'] . " ";
			if (isset($data['start']) || isset($data['limit'])) {
				if ($data['start'] < 0) {
		...
		{
			$stock_status_data = $this->cache->get('stock_status.' . (int)$config_language_id);
			if (!$stock_status_data) 
			{
				$strSql = "SELECT 
					...
					WHERE `language_id` = '" . (int)$config_language_id . "' 
					ORDER BY name ";
				$query = $this->db->query($strSql);
				$stock_status_data = $query->rows;
				$this->cache->set('stock_status.' . (int)$config_language_id, $stock_status_data);
			}
	...
	public function getLengthClasses($data = array()) 
	{
		...
		{
			$sql = "SELECT * 
				FROM `" . DB_PREFIX . "length_class` AS lc 
				LEFT JOIN `" . DB_PREFIX . "length_class_description` AS lcd 
				ON (lc.length_class_id = lcd.length_class_id) 
				WHERE lcd.language_id = '" . (int)$config_language_id . "' ";
			$sort_data = array(
				'title',
				'unit',
				'value'
		...
		{
			$length_class_data = $this->cache->get('length_class.' . (int)$config_language_id);
			if (!$length_class_data) {
				$strSql = "SELECT * 
					...
					ON (lc.length_class_id = lcd.length_class_id) 
					WHERE 
						`lcd.language_id` = '" . (int)$config_language_id . "' ";
				$query = $this->db->query($strSql);
				$length_class_data = $query->rows;
				$this->cache->set('length_class.' . (int)$config_language_id, $length_class_data);
			}
	...
	public function search($data = array(), $request, $customer) 
	{
		...
		$sql = "SELECT 
			...
			WHERE 
				...
				AND p2s.store_id = '" . (int)$config_store_id . "' 
		";
		if(isset($request['filters'])) $sql.= $this->searchHelper($request);
		$sql .= " GROUP BY p.product_id ";
	...
	public function getProducts($data = array()) {
		...
		if (!empty($data['filter_category_id'])) {
			...
			if ((!empty($data['filter_name'])) && $this->config->get('module_extended_search_status') && $this->config->get('module_extended_search_attr'))
		...
		if (!empty($data['filter_name']) || !empty($data['filter_tag'])) {
			...
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'model')) $es .= " OR LCASE(p.model) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'sku'))$es .= " OR LCASE(p.sku) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'upc'))$es .= " OR LCASE(p.upc) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'ean'))$es .= " OR LCASE(p.ean) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'jan'))$es .= " OR LCASE(p.jan) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'isbn'))$es .= " OR LCASE(p.isbn) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'mpn'))$es .= " OR LCASE(p.mpn) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'location'))$es .= " OR LCASE(p.location) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			if ($this->config->get($temp_es.'status') && $this->config->get($temp_es.'attr'))$es .= " OR LCASE(pa.text) LIKE '%" . $this->db->escape(utf8_strtolower($word)) . "%'";
			$es .= ") ";
			$implode[] = $es;
			...
			if (!$this->config->get('module_extended_search_status') && !$this->config->get('module_extended_search_model')) {
				$sql .= " OR LCASE(p.model) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
			
				
			if (!$this->config->get('module_extended_search_status') && !$this->config->get('module_extended_search_sku')) {
				$sql .= " OR LCASE(p.sku) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
			
				
			if (!$this->config->get('module_extended_search_status') && !$this->config->get('module_extended_search_upc')) {
				$sql .= " OR LCASE(p.upc) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
			
				
			if (!$this->config->get('module_extended_search_status') && !$this->config->get('module_extended_search_ean')) {
				$sql .= " OR LCASE(p.ean) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
			
				
			if (!$this->config->get('module_extended_search_status') && !$this->config->get('module_extended_search_jan')) {
				$sql .= " OR LCASE(p.jan) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
			
				
			if (!$this->config->get('module_extended_search_status') && !$this->config->get('module_extended_search_isbn')) {
				$sql .= " OR LCASE(p.isbn) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
			
				
			if (!$this->config->get('module_extended_search_status') && !$this->config->get('module_extended_search_mpn')) {
				$sql .= " OR LCASE(p.mpn) = '" . $this->db->escape(utf8_strtolower($data['filter_name'])) . "'";
			}
	...
	public function getProductOptions($product_id) {
		...
		foreach ($product_option_query->rows as $product_option) {
			...
			if (empty($product_option_value_data) &&  !in_array($product_option['type'], array('text','textarea','file','date','time','datetime'))) {continue;}
	...
	public function getProductNumberOfColor($product_id) {
		$query = $this->db->query("SELECT COUNT(po.product_option_id) as pcnt FROM " . DB_PREFIX . "product p INNER JOIN " . DB_PREFIX . "product_option po on p.product_id = po.product_id INNER JOIN " . DB_PREFIX . "product_option_value pov on po.product_option_id = pov.product_option_id INNER JOIN " . DB_PREFIX . "option_description od ON po.option_id = od.option_id WHERE p.product_id = " . (int)$product_id . " AND od.`name` = 'COLOR'");

		return $query->row['pcnt'];
	}
	...
	public function getCategories($product_id) {
		...
		return $query->rows;
			...
			$strQry = "SELECT 
				pc.*, 
				(!ISNULL(t1.parent_id) + !ISNULL(t2.parent_id) + !ISNULL(t3.parent_id) + !ISNULL(t4.parent_id) + !ISNULL(t5.parent_id))*1000 
				+ IF(t1.sort_order>0,(1000-t1.sort_order),0) 
				+ IF(t2.sort_order>0,(1000-t2.sort_order),0) 
				+ IF(t3.sort_order>0,(1000-t3.sort_order),0) 
				+ IF(t4.sort_order>0,(1000-t4.sort_order),0) 
				+ IF(t5.sort_order>0,(1000-t5.sort_order),0) AS d 
				...
				LEFT JOIN " . DB_PREFIX . "category t4 ON t3.parent_id = t4.category_id 
				LEFT JOIN " . DB_PREFIX . "category t5 ON t4.parent_id = t5.category_id 
				WHERE 
	...
	public function getOtherFitProduct($product_id) {
		
		$query = $this->db->query("SELECT model FROM " . DB_PREFIX . "product WHERE product_id = '" . (int)$product_id . "'");
		$model = $query->row['model'];

		if (strpos($model, 'X')) {
			$query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE model = '" . str_replace("X", "", $model) . "' AND status = 1");
			if ($query->num_rows == 0) {
				$other_fit_product_id = false;
			} else {
				$other_fit_product_id = $query->row['product_id'];
			}
		} else {
			if (strpos($model, '-')) {
				$query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE model = '" . str_replace("-", "X-", $model) . "' AND status = 1");
				...
				$query = $this->db->query("SELECT product_id FROM " . DB_PREFIX . "product WHERE model = '" . $model . "X" . "' AND status = 1");
				if ($query->num_rows == 0) {
					$other_fit_product_id = false;
				} else {
					$other_fit_product_id = $query->row['product_id'];
		...
		return $other_fit_product_id;
	}
	
	public function getMinPriceForVolumeDiscountTable($product_id) {					
    $strSql = "
      SELECT 
      ...
      WHERE 
        `product_id` = '" . (int)$product_id . "' 
        AND `option_id` = 1";
		$product_discount_query = $this->db->query($strSql);
		
		if($product_discount_query->num_rows) {
			return $product_discount_query->row['min_price'];
		} else {
			return "0";
		}
	}
