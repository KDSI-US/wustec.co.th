...
class ModelAccountOrder extends Model {
	...
	public function getOrdersByFilter($data = array()) 
	{
		$sql = "SELECT 
			...
			FROM `" . DB_PREFIX . "order` AS o 
			LEFT JOIN `" . DB_PREFIX . "country` AS payment_country 
				ON (payment_country.country_id = o.payment_country_id) 
			LEFT JOIN `" . DB_PREFIX . "country` AS shipping_country 
				ON (shipping_country.country_id = o.shipping_country_id) 
			LEFT JOIN `" . DB_PREFIX . "zone` AS payment_zone 
				ON (payment_zone.zone_id = o.payment_zone_id) 
			LEFT JOIN `" . DB_PREFIX . "zone` AS shipping_zone 
				ON (shipping_zone.zone_id = o.shipping_zone_id) 
		";
	...
	public function getOrderProductsForRowspan($order_id) {
		$strSql = "
			SELECT 
				op.*, 
				(select count(model) 
	...
	public function getOrderOptionNameColor($order_id, $order_product_id) {
		$strSql = "
			SELECT * 
	...
	public function getOrderOptionImage($product_option_value_id) {
		$strSql = "
			SELECT * 
	...
	public function getOrderTrackings($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_shipment os INNER JOIN " . DB_PREFIX . "shipping_courier sc ON os.shipping_courier_id = sc.shipping_courier_id WhERE order_id = '" . (int)$order_id . "' ORDER BY order_shipment_id DESC");

		return $query->rows;
	}

	public function getMaxOrderTrackingNumber($order_id) {
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_shipment WHERE order_id = '" . (int)$order_id . "' ORDER BY order_shipment_id DESC LIMIT 1");

		if ($query->rows) {
			return $query->row['tracking_number'];
		} else {
			return "";
		}
	}
