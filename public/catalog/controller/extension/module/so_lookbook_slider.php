...
class ControllerExtensionModuleSoLookBookSlider extends Controller {
	public function index($setting) {
		...
		if (!defined ('LOOKBOOK_OWL_CAROUSEL')){
			$this->document->addStyle('catalog/view/javascript/so_lookbook/css/animate.css');
			$this->document->addStyle('catalog/view/javascript/so_lookbook/css/owl.carousel.css');
			$this->document->addStyle('catalog/view/javascript/so_lookbook/css/owl.theme.min.css');
			$this->document->addScript('catalog/view/javascript/so_lookbook/js/owl.carousel.min.js');
		...
		if ($sliderId) {
			$slider = $this->model_extension_module_so_lookbook->getSliderById($sliderId);
			if ($slider['status'] == 1) {
				$lookbooks = $this->model_extension_module_so_lookbook->getLookBookBySlideId($sliderId);
				if (count($lookbooks)) {
		...
		return $this->load->view('extension/module/so_lookbook/lookbook_slide', $data);
	}
