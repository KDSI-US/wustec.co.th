...
class ControllerOcrestapiCheckoutPaymentAddress extends ocrestapicontroller {
			...
			$this->load->model('account/address');
				...
				if (empty($json)) {
					...
					$token = explode(' ', $get_token)[1];
			 		$this->add_payment_address_id($this->request->post['address_id'],$token);
					$this->json['data'] = [];
				}
	...
	public function add_payment_address_id($address_id,$token) {
		
		$this->load->model('ocrestapi/ocrestapi');
		$oauth_data = $this->model_ocrestapi_ocrestapi->get_auth_data($token);
	
		 $oauth_data['payment_address_id'] = $address_id;
		if(!empty($oauth_data['payment_address_id'])){	
		$this->session->data['payment_address_id']=$oauth_data['payment_address_id'];
