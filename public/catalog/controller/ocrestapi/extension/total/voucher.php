...
class ControllerOcrestapiExtensionTotalVoucher extends OcrestapiController {
	public function index() {
		$this->checkPlugin();
		if ($this->config->get('total_voucher_status')) {
			$language = $this->load->language('extension/total/voucher');
	...
	public function voucher() {
		...
		if (empty($this->request->post['voucher'])) {
			$this->json['errors']['voucher'] = $this->language->get('error_empty');
		} elseif ($voucher_info) {
			$this->session->data['voucher'] = $this->request->post['voucher'];
			$this->json['data']['success'] = $this->language->get('text_success');	
		} else {
			$this->json['errors']['voucher'] = $this->language->get('error_voucher');
		}
