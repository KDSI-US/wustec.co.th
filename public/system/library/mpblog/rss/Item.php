...
class Item
{
    ...
    */
    public function __construct($version = Feed::RSS2, $oc_registry)
    {
    ...
    */
    public function addElement($elementName, $content, array $attributes = null, $overwrite = FALSE, $allowMultiple = FALSE)
    {
        if (empty($elementName)) {
            $this->oc_registry->get('log')->write('The element name may not be empty or NULL.');
        }
        if (!is_string($elementName)) {
            $this->oc_registry->get('log')->write('The element name must be a string.');
        }
    ...
    public function getElements()
    {
        ...
        if ($this->version == Feed::ATOM)
        {
            ...
            if (!array_key_exists('content', $this->elements) && !array_key_exists('link', $this->elements))
                $this->oc_registry->get('log')->write('ATOM feed entries need a link or a content element. Call the setLink or setContent method.');
        }
        // ...same with RSS1 feeds.
        else if ($this->version == Feed::RSS1)
        {
            if (!array_key_exists('title', $this->elements))
                $this->oc_registry->get('log')->write('RSS1 feed entries need a title element. Call the setTitle method.');
            if (!array_key_exists('link', $this->elements))
                $this->oc_registry->get('log')->write('RSS1 feed entries need a link element. Call the setLink method.');
        }
    ...
    */
    ...
    {
        if ($this->version != Feed::ATOM)
            $this->oc_registry->get('log')->write('The content element is supported in ATOM feeds only.');
    ...
    public function setDate($date)
    {
        if (!is_numeric($date)) {
            if ($date instanceof DateTime)
                ...
                if ($date === FALSE)
                    $this->oc_registry->get('log')->write('The given date string was not parseable.');
            }
        } elseif ($date < 0)
            $this->oc_registry->get('log')->write('The given date is not an UNIX timestamp.');
    ...
    */
    public function addEnclosure($url, $length, $type, $multiple = TRUE)
    {
        if ($this->version == Feed::RSS1)
            $this->oc_registry->get('log')->write('Media attachment is not supported in RSS1 feeds.');
        ...
        if (!is_numeric($length) || $length < 0)
            $this->oc_registry->get('log')->write('The length parameter must be an integer and greater or equals to zero.');

        // Regex used from RFC 4287, page 41
        if (!is_string($type) || preg_match('/.+\/.+/', $type) != 1)
            $this->oc_registry->get('log')->write('type parameter must be a string and a MIME type.');
    ...
    */
    ...
    {
        if ($this->version == Feed::RSS1)
            $this->oc_registry->get('log')->write('The author element is not supported in RSS1 feeds.');

        // Regex from RFC 4287 page 41
        if ($email != null && preg_match('/.+@.+/', $email) != 1)
            $this->oc_registry->get('log')->write('The email address is syntactically incorrect.');
    ...
    public function setId($id, $permaLink = false)
    {
        if ($this->version == Feed::RSS2) {
            if (!is_bool($permaLink))
                $this->oc_registry->get('log')->write('The permaLink parameter must be boolean.');
            ...
            if (!$found)
                $this->oc_registry->get('log')->write("The ID must begin with an IANA-registered URI scheme.");

            $this->addElement('id', $id, NULL, TRUE);
        } else
            $this->oc_registry->get('log')->write('A unique ID is not supported in RSS1 feeds.');
