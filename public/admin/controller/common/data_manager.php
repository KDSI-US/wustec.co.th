<?php
class ControllerCommonDataManager extends Controller {
	public function index() {
		$this->load->language('common/data_manager');
		...
		foreach ($images as $image) {
			...
			if (is_dir($image)) {
				...
				$data['images'][] = array(
					...
					'href'  => $this->url->link('common/data_manager', 'user_token=' . $this->session->data['user_token'] . '&directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE . 'catalog/'))) . $url, true)
				);
		...
		$data['parent'] = $this->url->link('common/data_manager', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$data['refresh'] = $this->url->link('common/data_manager', 'user_token=' . $this->session->data['user_token'] . $url, true);
		...
		$this->load->model('localisation/language');
        	...
        	$data['image_title_status'] = $this->config->get('module_data_manager_image_title_status');
        	$data['image_description_status'] = $this->config->get('module_data_manager_image_description_status');
		...
		$pagination->limit = 16;
		$pagination->url = $this->url->link('common/data_manager', 'user_token=' . $this->session->data['user_token'] . $url . '&page={page}', true);

		$data['pagination'] = $pagination->render();

		$this->response->setOutput($this->load->view('common/data_manager', $data));
	}
	...
	public function upload() {
		...
		if (!$this->user->hasPermission('modify', 'common/data_manager')) {
