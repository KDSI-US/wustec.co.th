...
function obuiErrorHandler($errno, $errstr, $errfile, $errline, $errcontext = array()) {
...
class ModelToolUniversalImport extends Model {
  ...
  private $deletedIdFile;
  private $noDeleteFile;
  private $noDeleteBrandFile;
  private $processedIdArray;
  private $deletedIdArray;
  private $noDeleteArray;
  private $noDeleteBrandArray;
  ...
  public function process($config, $limit = null) {
    ...
    $this->deletedIdFile = DIR_CACHE.'universal_import/'.md5($processedIdFilename).'.del';
    $this->noDeleteFile = DIR_CACHE.'universal_import/'.md5($processedIdFilename).'.nodel';
    $this->noDeleteBrandFile = DIR_CACHE.'universal_import/'.md5($processedIdFilename).'.nodelbrand';
    
    $config['row_start'] = (int) $config['row_start'];
    
    // first init
    if (empty($this->session->data['obui_current_line'])) {
      // delete identifier files
      $this->deleteIdentifierFile('processedId');
      $this->deleteIdentifierFile('deletedId');
      
      //$this->session->data['obui_deleted_ids'] = array(); // file mode loaded after
      ...
      if (!empty($config['delete'])) {
        if ($config['delete'] == 'all' || $config['delete'] == 'batch') {
          ...
          $this->deleteIdentifierFile('noDelete');
          $this->deleteIdentifierFile('noDeleteBrand');
        }
      }
    ...
    $this->loadIdentifierFile('processedId');
    $this->loadIdentifierFile('deletedId');
    $this->loadIdentifierFile('noDelete');
    $this->loadIdentifierFile('noDeleteBrand');
        ...
        try {
          ...
          foreach ($resArray as $res) {
            $this->tool->log(array(
              ...
              'title' => isset($res['row_status']) ? $this->language->get('text_'.$simu_row.'_'.$res['row_status']) : '',
  ...
  public function delete($config) {
    ...
    if (!empty($config['delete_notempty']) && empty($this->session->data['obui_processed']['processed'])) {
      $this->tool->log(array(
        'row' => '',
        'status' => 'deleted',
        'title' => $this->language->get('text_'.$simu_row.'_deleted'),
        'msg' => 'Delete items aborted because file is empty',
      ));
    ...
    if ($mode == 'missing' && !empty($this->noDeleteArray)) {
      $where = " WHERE " . $type . "_id NOT IN ('" . implode("','", array_map('addslashes', $this->noDeleteArray)) . "')";
    } else if ($mode == 'batch') {
      $where = " WHERE `import_batch` = '" . $this->db->escape($config['delete_batch']) . "'";
    } else if ($mode == 'missing_brand') {
      if (!empty($this->noDeleteArray) && !empty($this->noDeleteBrandArray)) {
        $where = " WHERE " . $type . "_id NOT IN ('" . implode("','", array_map('addslashes', $this->noDeleteArray)) . "') AND manufacturer_id IN ('" . implode("','", $this->noDeleteBrandArray) . "')";
      } else {
    ...
    */
    ...
    if ($action == 'delete') {
      ...
      $this->session->data['obui_processed']['disabled'] += count($deleted_array);
    } else if ($action == 'disable_zero') {
      foreach ($to_delete as $del) {
        if (!$this->simulation) {
          $this->db->query("UPDATE " . DB_PREFIX . $type . " SET status = 0, quantity = 0 WHERE " . $type . "_id = '" . (int) $del[$type.'_id'] . "'");
        }
      ...
      $this->session->data['obui_processed']['disabled'] += count($deleted_array);
    } else if ($action == 'zero_opt') {
      foreach ($to_delete as $del) {
        if (!$this->simulation) {
          $this->db->query("UPDATE " . DB_PREFIX . $type . " SET quantity = 0 WHERE " . $type . "_id = '" . (int) $del[$type.'_id'] . "'");
          $this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = 0 WHERE product_id = '" . (int) $del['product_id'] . "'");
        }
      ...
      $this->session->data['obui_processed']['disabled'] += count($deleted_array);
    } else if ($action == 'disable_zero_opt') {
      foreach ($to_delete as $del) {
        if (!$this->simulation) {
          $this->db->query("UPDATE " . DB_PREFIX . $type . " SET status = 0, quantity = 0 WHERE " . $type . "_id = '" . (int) $del[$type.'_id'] . "'");
          $this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET quantity = 0 WHERE product_id = '" . (int) $del['product_id'] . "'");
    ...
    } else if (in_array($action, array('zero', 'zero_opt', 'disable_zero', 'disable_zero_opt'))) {
      $this->tool->log(array(
        'row' => '',
        'status' => 'disabled',
  ...
  public function process_product($config, $line) {
    ...
    //@todo: for further items returns not the item id but the identifier
          ...
          if ($func_name == 'delete_item') {
            ...
            if (($func_values['comparator'] == 'is_equal' && $func_values['field'] == $func_values['value']) ||
              ...
              if ($item_id && !empty($config['delete']) && $config['delete'] != 'all' && !empty($config['no_delete_skipped'])) {
                $this->addProcessedIdentifier('noDelete', $item_id);
              }
            ...
            if ($func_values['db_field'] == 'special' && !isset($item_data['special'])) {
              $specialPrice = $this->db->query("SELECT price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = ".(int) $item_id." AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1")->row;
              
              if (!empty($specialPrice['price'])) {
                $item_data['special'] = (float) $specialPrice['price'];
              }
            }
            
            if (!array_key_exists($func_values['db_field'], $item_data)) continue;
    ...
    if (($item_id && $config['item_exists'] == 'soft_update') || ($item_id && $config['item_exists'] == 'update') || ($item_id && $config['item_exists'] == 'option') || (!$item_id && $config['item_not_exists'] == 'insert')) {
      ...
      if ($item_id && $config['item_exists'] == 'soft_update') {
        ...
        // disable update preserve field
        if (!empty($config['disable_update'])) {
          $toDisable = explode(',', $config['disable_update']);
          
          foreach ($toDisable as $disableField) {
            if (isset($config['columns'][trim($disableField)])) {
              unset($config['columns'][trim($disableField)]);
            }
          }
        }
        
        if (!empty($config['disable_update_empty'])) {
          $toDisable = explode(',', $config['disable_update_empty']);
          
          foreach ($toDisable as $disableField) {
            if (isset($config['columns'][trim($disableField)]) && $config['columns'][trim($disableField)] === '') {
              unset($config['columns'][trim($disableField)]);
            }
          }
        }
      ...
      */
      ...
      try {
        ...
        if (isset($data['image']) && $config['item_identifier'] != 'image') {
          if (empty($config['image_insert_type']) || (!empty($config['image_insert_type']) && $config['image_insert_type'] == 'add') || (!empty($config['image_insert_type']) && $config['image_insert_type'] == 'rm_add' && !(in_array($item_id, $this->session->data['obui_processed_ids']) || $this->identifierExists('processedId', $item_id)))) {
        ...
        if (isset($data['manufacturer_id']))      $data['manufacturer_id'] = $this->handler->manufacturerHandler($config, $noDeleteBrands);
        ...
        if (isset($data['product_option']) ||
          !empty($config['option_fields']))       $data['product_option'] = $this->handler->optionHandler('product_option', $config, $line, $item_id, !empty($is_option)); // 
        ...
        if (isset($data['product_recurring']))    $data['product_recurring'] = $this->handler->recurringHandler('product_recurring', $config);
        if (isset($data['status']))               $data['status'] = $this->handler->booleanHandler('status', $config); // enabled/disabled, on/off, true/false, 1/0
        
        // save noDeleteBrands
        foreach ($noDeleteBrands as $noDeleteBrand) {
          $this->addProcessedIdentifier('noDeleteBrand', $noDeleteBrand);
        }
      ...
      */
      ...
      // if new item or replace mode auto set some default values if empty
      if (!$item_id || $config['item_exists'] != 'soft_update' && !empty($data['product_description'])) {
        foreach ($data['product_description'] as $language_id => &$item_desc) {
          // meta title
          if (!$this->config->get('mlseo_insertautoseotitle')) {
            if (empty($item_desc['meta_title']) && !empty($item_desc['name'])) {
              $item_desc['meta_title'] = $item_desc['name'];
            }
          }
          
          // meta desc
          if (!$this->config->get('mlseo_insertautometadesc')) {
            if (empty($item_desc['meta_description']) && !empty($item_desc['description'])) {
              if (function_exists('mb_substr')) {
                $item_desc['meta_description'] = trim(preg_replace('/\s\s+/', ' ', mb_substr(strip_tags(str_replace(array('</p>', '</li>'), array('</p> ', '</li> '), $item_desc['description'])), 0, 220)));
              } else {
                $item_desc['meta_description'] = trim(preg_replace('/\s\s+/', ' ', substr(strip_tags(str_replace(array('</p>', '</li>'), array('</p> ', '</li> '), $item_desc['description'])), 0, 220)));
              }
            }
          }
          
          // seo keyword
          if (!$this->config->get('mlseo_insertautourl')) {
            if (isset($item_desc['seo_keyword']) && empty($item_desc['seo_keyword']) && !empty($item_desc['name'])) {
              $item_desc['seo_keyword'] = $this->tool->urlify($item_desc['name']);
            } else if (isset($data['product_seo_url'][0][$language_id]) && empty($data['product_seo_url'][0][$language_id]) && !empty($item_desc['name'])) {
              $data['product_seo_url'][0][$language_id] = $this->tool->urlify($item_desc['name']);
            }
          }
        }
      }
        ...
        foreach ($extra_funcs as $func_name => $func_values) {
          if ($func_name == 'delete_item') {
            ...
            if (($func_values['comparator'] == 'is_equal' && $func_values['field'] == $func_values['value']) ||
              ...
              return $data;
            } else if (!empty($config['xfn_verbose'])) {
              $this->tool->log(array(
                'row' => $this->session->data['obui_current_line'],
                'status' => 'info',
                'title' => $this->language->get('text_rows_info'),
                'msg' => $this->language->get('text_condition_failed') . ': ' . (isset($func_values['orig_field']) ? $func_values['orig_field'] : $func_values['field']) . ' ' . strtolower($this->language->get('xfn_'.$func_values['comparator'])) . ' ' . (isset($func_values['orig_fieldval']) ? $func_values['orig_fieldval'] : $func_values['value']),
              ));
            }
          } else if ($func_name == 'skip_db') {
            if (!isset($item_data)) {
            ...
            if (($func_values['comparator'] == 'is_equal' && $item_data[$func_values['db_field']] == $func_values['value']) ||
              ...
              $this->tool->log(array(
                ...
                'status' => 'info',
                'title' => $this->language->get('text_rows_info'),
                'msg' => $this->language->get('text_condition_failed') . ': ' . strtolower($this->language->get('entry_'.$func_values['db_field'])) . ' ('.$item_data[$func_values['db_field']].') ' . strtolower($this->language->get('xfn_'.$func_values['comparator'])) . ' ' . (isset($func_values['orig_fieldval']) ? $func_values['orig_fieldval'] : $func_values['value']),
              ));
            }
    ...
    if (!empty($is_option)) {
      if (!empty($config['image_insert_type'])) {
        $newImages = $this->handler->imageHandler('product_image', $config, true, $item_id, false);
      }
      ...
      if (($item_id || ($this->simulation && $this->itemHasProcessed($config['import_type'], $config['item_identifier'], $data))) && !empty($data['product_option'])) {
       if (!empty($config['image_insert_type'])) {
          ...
          if (!$this->simulation) {
            ...
            $this->model_gkd_import_product->addProductImages($item_id, $newImages);
          }
        }
    ...
    if ($item_id) {
      ...
      if ($this->simulation || !empty($saveIdentifier)) {
        ...
        if (in_array($config['item_identifier'], array('name', 'title', 'description')) && isset($data['product_description']) && $prodDesc = reset($data['product_description'])) {
          $this->addProcessedIdentifier('processedId', $prodDesc[$config['item_identifier']]);
        } else {
          $this->addProcessedIdentifier('processedId', (isset($data[$config['item_identifier']]) ? $data[$config['item_identifier']] : $item_id));
        }
        //$this->addProcessedIdentifier('processedId', $item_id);
      }
    } else {
    ...
    if ($item_id) {
      foreach ($itemsArray as $item_id) {
        ...
        if (empty($config['option_identifier']) && !empty($data['option_identifier_value'])) {
          ...
          } else if (isset($data['quantity']) && !empty($config['multiple_separator'])) {
            $quantity_array = explode(@html_entity_decode($config['multiple_separator'], ENT_QUOTES, 'UTF-8'), $data['quantity']);
          } else {
            $quantity_array = false;
          }
          ...
          foreach ($values_array as $currentKey => $optIdVal) {
            ...
            if (!$this->simulation) {
              ...
              if (isset($k) && isset($quantity_array[$k])) {
                ...
                $this->db->query("UPDATE " . DB_PREFIX . "product_option_value SET " . $update_quantity . " WHERE product_option_value_id = '" . (int)$opt_query['product_option_value_id'] . "'");
              } else if (isset($currentKey) && isset($quantity_array[$currentKey])) {
                $update_quantity = array();
                
                if (empty($config['quantity_modifier'])) {
                  $update_quantity = "quantity = '" . (int) $quantity_array[$currentKey] . "' ";
                } else if ($config['quantity_modifier'] == '+') {
                  $update_quantity = "quantity = quantity + '" . (int) $quantity_array[$currentKey] . "' ";
                } else if ($config['quantity_modifier'] == '-') {
                  $update_quantity = "quantity = quantity - '" . (int) $quantity_array[$currentKey] . "' ";
                }
      ...
      if (false) {
        ...
        if (isset($data[$config['item_identifier']])) {
          $message .=  ' (' . $data[$config['item_identifier']].')';
        }
  ...
  public function process_order_info($config, $line) {
    ...
    */
    ...
    if (isset($data['customer_id']))        $data['customer_id'] = $this->handler->customerHandler($data['customer_id'], $config);
  ...
  public function process_seo_redirect($config, $line) {
    $this->pre_process($config);
    ...
    */
    ...
    if (empty($data['query'])) {
      ...
      $data['row_msg'] = $this->language->get('text_rows_skipped') . ' - From field is empty';
      $data['error'] = $this->language->get('text_rows_skipped') . ' - From field is empty';
      $this->session->data['obui_processed']['processed']++;
    ...
    if (empty($data['redirect'])) {
      $data['row_status'] = 'error';
      $this->session->data['obui_processed']['error']++;
      $data['row_msg'] = $this->language->get('text_rows_skipped') . ' - Redirect to field is empty';
      $data['error'] = $this->language->get('text_rows_skipped') . ' - Redirect to field is empty';
      $this->session->data['obui_processed']['processed']++;
    ...
    $itemExists = $this->db->query("SELECT url_redirect_id FROM " . DB_PREFIX . "url_redirect WHERE query = '" . $this->db->escape($data['query']) . "' LIMIT 1")->row;
    
    if (!empty($itemExists['url_redirect_id'])) {
      $item_id = $itemExists['url_redirect_id'];
    }
    ...
    //$data['language_id'] = $this->handler->languageHandler('language_id', $config);
    
    if ($item_id) {
      if (!$this->simulation) {
        $this->db->query("UPDATE " . DB_PREFIX . "url_redirect SET redirect = '" . $this->db->escape($data['redirect']) . "', language_id = " . (int) $data['language_id'] . " WHERE url_redirect_id = " . (int) $item_id);
      }
      ...
      if (!$this->simulation) {
        $this->db->query("INSERT INTO " . DB_PREFIX . "url_redirect SET query = '" . $this->db->escape($data['query']) . "', redirect = '" . $this->db->escape($data['redirect']) . "', language_id = " . (int) $data['language_id']);
        $item_id = $this->db->getLastId();
      }
    ...
    */
    ...
    if ($item_id) {
      if (defined('GKD_CRON')) {
        $data['row_msg'] = $this->language->get('text_type_seo_redirect').' #'.$item_id . ' - '. $data['query'];
      } else {
        $data['row_msg'] = $data['query'];
      }
    } else {
      $data['row_msg'] = $data['query'];
    }
    ...
    */
    ...
    if (($item_id && $config['item_exists'] == 'soft_update') || ($item_id && $config['item_exists'] == 'update') || (!$item_id && $config['item_not_exists'] == 'insert')) {
      ...
      */
      ...
      // if new item or replace mode auto set some default values if empty
      if (!$item_id || $config['item_exists'] != 'soft_update' && !empty($data['category_description'])) {
        foreach ($data['category_description'] as $language_id => &$item_desc) {
          // meta title
          if (empty($item_desc['meta_title']) && !empty($item_desc['name'])) {
            $item_desc['meta_title'] = $item_desc['name'];
          }
          ...
          if (!$this->config->get('mlseo_insertautourl')) {
            if (isset($item_desc['seo_keyword']) && empty($item_desc['seo_keyword']) && !empty($item_desc['name'])) {
              $item_desc['seo_keyword'] = $this->tool->urlify($item_desc['name']);
            } else if (isset($data['category_seo_url'][0][$language_id]) && empty($data['category_seo_url'][0][$language_id]) && !empty($item_desc['name'])) {
              $data['category_seo_url'][0][$language_id] = $this->tool->urlify($item_desc['name']);
            }
          }
        }
      }
    ...
    if ($item_id) {
      ...
      if ($config['item_not_exists'] == 'insert') {
        ...
        if ($this->simulation) {
          //$this->addProcessedIdentifier('processedId', $data[$config['item_identifier']]);
  ...
  public function process_information($config, $line) {
    ...
    if (($item_id && $config['item_exists'] == 'update') || (!$item_id && $config['item_not_exists'] == 'insert')) {
      ...
      // if new item or replace mode auto set some default values if empty
      if (!$item_id || $config['item_exists'] != 'soft_update' && !empty($data['information_description'])) {
        foreach ($data['information_description'] as $language_id => &$item_desc) {
          // meta title
          if (empty($item_desc['meta_title']) && !empty($item_desc['title'])) {
            $item_desc['meta_title'] = $item_desc['title'];
          }
          ...
          // seo keyword
          if (!$this->config->get('mlseo_insertautourl')) {
            if (isset($item_desc['seo_keyword']) && empty($item_desc['seo_keyword']) && !empty($item_desc['title'])) {
              $item_desc['seo_keyword'] = $this->tool->urlify($item_desc['title']);
            } else if (isset($data['information_seo_url'][0][$language_id]) && empty($data['information_seo_url'][0][$language_id]) && !empty($item_desc['title'])) {
              $data['information_seo_url'][0][$language_id] = $this->tool->urlify($item_desc['title']);
            }
          }
        }
      }
    ...
    if ($mode == 'product') {
      foreach ($product_ids as $product_id) {
        ...
        if (!empty($config['delete_attributes'])) {
          if (!$this->simulation && !$this->identifierExists('deletedId', $product_id)) {
        ...
        */
        ...
        //if (empty($config['attr_insert_type']) || (!empty($config['attr_insert_type']) && $config['attr_insert_type'] == 'rm_add' && !$this->identifierExists('processedId', $product_id))) {
        if (empty($config['attr_insert_type']) || (!empty($config['attr_insert_type']) && $config['attr_insert_type'] == 'rm_add' && !$this->identifierExists('deletedId', $product_id))) {
          if (!$this->simulation) {
            $this->db->query("DELETE FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "'");
          }
          ...
          $this->tool->log(array(
            'row' => $this->session->data['obui_current_line'],
            'status' => 'deleted',
            'title' => $this->language->get('text_rows_deleted'),
            'msg' => 'Cleared attributes for product ID: '.$product_id,
          ));
          
          //$this->session->data['obui_deleted_ids'][] = $product_id;
          $this->addProcessedIdentifier('deletedId', $product_id);
        }
        ...
        if ($product_id) {
          if ($data['row_status'] != 'skipped') {
            if (!$this->simulation) {
              foreach ($data['attribute_description'] as $language_id => $attribute_description) {
                $attributeExists = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_attribute WHERE product_id = '" . (int)$product_id . "' AND attribute_id = '" . (int)$item_id . "' AND language_id = '" . (int)$language_id . "'")->row;
                ...
                if (empty($attributeExists)) {
                  ...
                  $this->db->query("UPDATE " . DB_PREFIX . "product_attribute SET text = '" .  $this->db->escape($attribute_description['value']) . "' WHERE product_id = '" . (int)$product_id . "' AND attribute_id = '" . (int)$item_id . "' AND language_id = '" . (int)$language_id . "'");
                }
              }
            }
          }
      ...
      if (defined('GKD_CRON')) {
        ...
        if (!empty($attrArray_ids)) {
          ...
          foreach ($attrArray_ids as $k => $id) {
            $data['row_msg'] .= '['.$id.'] <a target="_blank" href="'.$this->url->link('catalog/attribute/edit', 'attribute_id='.$id.'&' . $this->token, 'SSL').'">'.(is_array($mainValues['name']) ? $mainValues['name'][$k] : $mainValues['name']).'</a><br/>';
          }
        } else {
  ...
  */
    ...
    if (!empty($config['extra_func'])) {
      foreach ($config['extra_func'] as &$extra_funcs) {
        foreach ($extra_funcs as $func_type => &$func) {
          ...
          if (!isset($func['field']) || $func['field'] !== '' || in_array($func_type, array('date_convert'))) {
            ...
            if ($target !== false && is_array($line) && !isset($line[$target]) && !strpos($target, '*')) {
              $line[$target] = '';
            }
            
            if (isset($func['fieldval']) && $func['fieldval'] !== '') {

              if (isset($line[$func['fieldval']])) {
                $value = $line[$func['fieldval']];
              } else {
                $value = $this->model_gkd_import_handler->getArrayPath($line, $func['fieldval']);
              }
            } else {
            ...
            if ($func_type == 'add') {
              if (is_array($init_value)) {
                foreach ($init_value as &$val) {
                  $val = (float) $value + (float) $val;
                }
                
              } else {
                $init_value = (float) $value + (float) $init_value;
              }
              
              $line = $this->model_gkd_import_handler->setArrayPath($line, $target, $init_value);
              ...
              if (is_array($init_value)) {
                ...
                $init_value = (float) $init_value - (float) $value;
              }
              ...
              if (is_array($init_value)) {
                foreach ($init_value as &$val) {
                  $val = (1 + (float) $value / 100) * (float) $val;
                }
                
              } else {
                $init_value = (1 + (float) $value / 100) * (float) $init_value;
              }
              ...
              if (is_array($init_value)) {
                foreach ($init_value as &$val) {
                  if ((float) $value > 0) {
                    $val = round((float) $val, (int) $value);
                  }
                }
              } else {
                $init_value = round((float) $init_value, (int) $value);
              }
              ...
              if (strpos($value, '[') !== false) {
                ...
                foreach ($wholesaleTableCats as $wholesaleTableCats) {
                  ...
                  if (isset($config['columns']['product_category'][0])) {
                    $currentCat = $this->model_gkd_import_handler->getArrayPath($line, $config['columns']['product_category'][0]);
                  } else {
                    $currentCat = '';
                  }
                  
                  if (!empty($currentCat)) {
                    if ($currentCat == $catName || $catName == '*') {
                      $value = $catTable;
                      $processWholesale = true;
                      break;
                    }
            ...
            // String
              ...
              $line[$target] = mb_convert_case($init_value, MB_CASE_UPPER, 'UTF-8');
            } else if ($func_type == 'lowercase') {
              $line[$target] = mb_convert_case($init_value, MB_CASE_LOWER, 'UTF-8');
            } else if ($func_type == 'ucfirst') {
              $line[$target] = mb_convert_case(mb_substr($init_value, 0, 1), MB_CASE_UPPER, 'UTF-8') . mb_convert_case(mb_substr($init_value, 1), MB_CASE_LOWER, 'UTF-8');
            } else if ($func_type == 'ucwords') {
              $line[$target] = mb_convert_case($init_value, MB_CASE_TITLE, 'UTF-8');
              ...
              if (is_array($line[$target])) {
                ...
                // exiting value
                if (strpos($line[$target], '[') !== false && $config['import_type'] == 'product' && !empty($line[$config['columns'][$config['item_identifier']]])) {
                  if (!isset($item_data)) {
                    $item_data = $this->db->query("SELECT * FROM " . DB_PREFIX . "product WHERE `" . $this->db->escape($config['item_identifier']). "` = '" . $this->db->escape($line[$config['columns'][$config['item_identifier']]]) . "'")->row;
                  }
                  ...
                  if (strpos($line[$target], '[manufacturer]') !== false) {
                    $brand = $this->db->query("SELECT * FROM " . DB_PREFIX . "manufacturer WHERE `manufacturer_id` = '" . (int) $item_data['manufacturer_id'] . "'")->row;
                    
                    $replace['[manufacturer]'] = '';
                    
                    if (!empty($brand['name'])) {
                      $replace['[manufacturer]'] = $brand['name'];
                    }
                  }
                  
                  if (strpos($line[$target], '[name') !== false) {
                    $prodDesc = $this->db->query("SELECT * FROM " . DB_PREFIX . "product_description WHERE `product_id` = '" . (int) $item_data['product_id'] . "'")->rows;
                    
                    foreach ($prodDesc as $desc) {
                      $replace['[name'.$desc['language_id'].']'] = $desc['name'];
                    }
                  }
                  
                  if (strpos($line[$target], '[category') !== false) {
                    if (!empty($item_data['product_id'])) {
                      $category = $this->db->query("
                        ...
                        WHERE product_id = '" . (int)$item_data['product_id'] . "' ORDER BY cp.level DESC LIMIT 1")->row;
                      
                      $replace['[category_id]'] = !empty($category['category_id']) ? $category['category_id'] : '';
                      $replace['[category]'] = !empty($category['name']) ? $category['name'] : '';
                    } else {
                      $replace['[category_id]'] = '';
                      $replace['[category]'] = '';
                    }
              ...
              if (is_array($line[$target])) {
                foreach ($line[$target] as &$arrValue) {
                  if (is_scalar($arrValue)) {
                    ...
                    foreach ($arrValue as &$subArrValue) {
                      if (is_scalar($subArrValue)) {
                        $subArrValue = str_replace($value, htmlspecialchars_decode($value2), htmlspecialchars_decode($subArrValue));
                      } else if (is_array($subArrValue)) {
                        foreach ($subArrValue as &$subArrValue2) {
                          if (is_scalar($subArrValue2)) {
                            $subArrValue2 = str_replace($value, htmlspecialchars_decode($value2), htmlspecialchars_decode($subArrValue2));
                          } else if (is_array($subArrValue2)) {
                            foreach ($subArrValue2 as &$subArrValue3) {
                              if (is_scalar($subArrValue3)) {
                                $subArrValue3 = str_replace($value, htmlspecialchars_decode($value2), htmlspecialchars_decode($subArrValue3));
                              }
                            }
                          }
                        }
            ...
            // Web
              ...
              if (!$this->user->hasPermission('modify', 'module/universal_import')) {
                $this->tool->log(array(
                  ...
                  'status' => 'warning',
                  'title' => 'Warning',
                  'msg' => 'Cannot get data from remote content in demo, please contact us if you need to check your settings',
                ));
                
                $line[$target] = '[Data from remote content]';
              ...
              if (!$this->user->hasPermission('modify', 'module/universal_import')) {
                $this->tool->log(array(
                  ...
                  'status' => 'warning',
                  'title' => 'Warning',
                  'msg' => 'Cannot get data from remote API in demo, please contact us if you need to check your settings',
                ));
                
                $line[$target] = '[Data from remote API]';
            ...
            } else if ($func_type == 'jsonencode') {
              $line[$target] = json_encode($init_value);
            } else if ($func_type == 'jsondecode') {
              $line[$target] = json_decode($init_value, true);
              
              $getPath = isset($func['path']) ? $func['path'] : '';
              
              if (!empty($getPath) && isset($line[$target][$getPath])) {
                $line[$target] = $line[$target][$getPath];
              }
              
            // Extra
            } else if ($func_type == 'date_convert') {
              $date_format = !empty($func['to']) ? $func['to'] : 'Y-m-d H:i:s';
              $delay = !empty($func['delay']) ? $func['delay'] : 'today';
              
              if (!$init_value) {
                $line[$target] = date($date_format, strtotime($delay));
              } else if ($func['format'] == 'sql') {
                $line[$target] = date($date_format, strtotime($delay, strtotime($init_value)));
              } else if ($func['format'] == 'us') {
                $line[$target] = date($date_format, strtotime($delay, strtotime(str_replace('-', '/', $init_value))));
              } else if ($func['format'] == 'xls') {
                $line[$target] = date($date_format, strtotime($delay, ((int) $init_value-25569)*86400));
              } else {
                $line[$target] = date($date_format, strtotime($delay, strtotime(str_replace('/', '-', $init_value))));
              }
            }
            ...
            */
            ...
            if (in_array($func_type, array('skip'))) {
              $func['field'] = $init_value;
            } else if (isset($func['field'])) {
    ...
    if (!empty($config['defaults'])) {
      foreach ($config['defaults'] as $key => &$val) {
        ...
        if (isset($config['columns'][$key]) && $config['columns'][$key] === '') {
          ...
          $config['columns'][$key] = is_string($val) ? trim($val) : $val;
        } else if (is_array($val) && isset($config['columns'][$key]) && is_array($config['columns'][$key])) {
          foreach ($val as $k => $v) {
            if (isset($config['columns'][$key][$k]) && $config['columns'][$key][$k] === '') {
              $config['columns'][$key][$k] = is_string($v) ? trim($v) : $v;
            }
          }
        }
      }
  ...
  protected function itemExists($type, &$config, &$data, $key = NULL, $multiple = false) {
    ...
    foreach ((array) $values as $value) {
      ...
      if ($this->simulation && $this->identifierExists('processedId', $value)) {
        return $value;
      }
  ...
  protected function itemHasProcessed($type, $field, &$data, $key = NULL) {
    ...
    foreach ((array) $values as $value) {
      ...
      if ($this->identifierExists('processedId', $value)) {
        return $value;
      }
    }
  ...
  protected function addProcessedIdentifier($type, $value) {
    if (is_array($this->{$type.'Array'})) {
      // file mode - cannot use session for that
      if (!in_array($value, $this->{$type.'Array'})) {
        $this->{$type.'Array'}[] = $value;
        file_put_contents($this->{$type.'File'}, $value."\n", FILE_APPEND | LOCK_EX);
      }
    } else {
      // data verification mode (10 rows)
      $this->session->data['obui_'.$type][] = $value;
    }
  }
  
  protected function identifierExists($type, $value) {
    if (!empty($this->{$type.'Array'})) {
      if (in_array($value, $this->{$type.'Array'})) {
        return true;
      }
    } else if (!empty($this->session->data['obui_'.$type]) && in_array($value, $this->session->data['obui_'.$type])) {
      return true;
    }
  ...
  protected function loadIdentifierFile($type) {
    if (file_exists($this->{$type.'File'})) {
      $this->{$type.'Array'} = file($this->{$type.'File'}, FILE_IGNORE_NEW_LINES);
    } else {
      $this->{$type.'Array'} = array();
    }
  }
  
  protected function deleteIdentifierFile($type) {
    unset($this->session->data['obui_'.$type]);
    
    if (file_exists($this->{$type.'File'})) {
      unlink($this->{$type.'File'});
    }
  }
  ...
  public function loadFile(&$file, $filetype = '', $currentSheet = 0, &$config = array()) {
    ...
    if (in_array($extension, array('csv_', 'sql'))) {
      ...
      if (substr($config['import_api_field'], 0, 1) == '/') {
        $fh = \JsonMachine\Items::fromFile($file, ['decoder' => new \JsonMachine\JsonDecoder\ExtJsonDecoder(true), 'pointer' => $config['import_api_field']]);
      } else {
        $fh = \JsonMachine\Items::fromFile($file, ['decoder' => new \JsonMachine\JsonDecoder\ExtJsonDecoder(true)]);
      }
      ...
      if (!empty($this->session->data['obui_current_line'])) {
        ...
        if (substr($nodeName, 0, 1) !== '>' && (!empty($config['xml_proc']) && $config['xml_proc'] == 'xri')) {
          ...
          */
          ...
          while ($file->read() && !($file->name === $nodeName && $file->nodeType == XMLReader::ELEMENT));
  ...
  public function getNextRow(&$file, &$config) {
    ...
    if ($this->filetype == 'csv_') {
      ...
      if (substr($config['import_api_field'], 0, 1) == '/') {
        ...
        $import_api_field = $config['import_api_field'];
      }
      ...
      if (!empty($import_api_field)) {
        ...
        foreach ($jsonArray as $json) {
          ...
          if (!empty($config['import_transformer'])) {
            ...
            // no multi-rows, return full json
            if (!is_array($json)) {
              if ($i > 1) return false;
              
              return json_decode(file_get_contents($config['import_file']), true);
            }
  ...
  public function getTotalRows($file, $has_header = false, $xml_node = '', $filetype = '', $currentSheet = 0, $config = array(), $forceSpout = false) {
    if (self::benchmark) {
      $start_time = time();
    }
    ...
    if (in_array($extension, array('csv_', 'sql'))) {
    ...
    } else if (in_array($extension, array('csv', 'txt', 'tsv', 'ods', 'xlsx_')) || ($extension == 'xlsx' && version_compare(phpversion(), '8', '>=')) || $forceSpout) { // use PHPExcel to count xlsx files as it is much faster
      // Spout
      if (version_compare(phpversion(), '7.1', '>')) {
        require_once DIR_SYSTEM.'library/Spout3/Autoloader/autoload.php';
        if (version_compare(phpversion(), '8', '<')) {
      ...
      if (!empty($import_api_field)) {
        ...
        foreach ($fh as $d) {
          // abandon count if taking too much time
          if (time() - $countTime > 5) {
            $i = -1;
            break;
          }
  ...
  public function Xpath2Array($xpath, $xml) {
    ...
    preg_match('/([\w\d_]+)\:/',$xpath, $match);
    
    if (!empty($match[1])) {
      $nodeName = $match[1];
      $xpath = str_replace($match[0], '', $xpath);
    } else {
      $nodeName = $xpath;
    }
    ...
    foreach ($qry as $k => $value) {
      ...
      foreach ($res as $key => $value) {
        if (isset($currentValue[$k][$key])) {
          $currentValue[$k][$key] = (array) $array['['.$xpath.']'][$k][$key];
          $currentValue[$k][$key][] = $value;
        } else {
          $currentValue[$k][$key] = $value;
        }
      }
    ...
    if (!$fullArray && is_array($currentValue) && count($currentValue) == 1) {
      $currentValue = array_shift($currentValue);
    }
    ...
    foreach ($namespaces as $ns => $nsKey) {
      ...
      foreach ($xml2 as $key => $value) {
        if (is_object($value) && strpos(get_class($value), 'SimpleXML') !== false) {
          if ($value->count()) {
            // do not show detail in case there is more than 5 subnodes
            /*
            if ($value->count() > 5) {
              $array[$nsKey.$key] = $this->XML2Array($value, 99);
              continue;
            }
            */
            ...
            if (!isset($array[$nsKey.$key]) || (isset($array[$nsKey.$key]) && is_array($array[$nsKey.$key]))) {
              if ($value->count() > 5) {
                // do not show detail in case there is more than 5 subnodes
                $array[$nsKey.$key][$m] = $this->XML2Array($value, 99);
              } else {
                $array[$nsKey.$key][$m] = $this->XML2Array($value, $level);
              }
            }
            
            if ($level < 2 && isset($array[$nsKey.$key][$m]) && is_array($array[$nsKey.$key][$m])) {
              $i = 1;
  ...
  public function getFeedCategories() {
    ...
    if ($extension == 'csv_') {
          ...
          foreach ((array) $config['columns'][$cat_field] as $key => $cat) {
            ...
            if (is_scalar($cat) && !isset($rows[$cat])) {
              ...
              foreach ($cat as $subkey => $subcat) {
                if (is_scalar($subcat) && !isset($rows[$subcat])) {
                  $rows[$subcat] = isset($profile['col_binding'][md5($subcat)]) ? $profile['col_binding'][md5($subcat)] : '';
                }
              }
            }
          }
      ...
      if (!empty($import_api_field)) {
        ...
        foreach ($jsonArray as $json) {
          //$rows[] = $json;
          $rawData[] = $this->model_gkd_import_tool->arrayFlat($json);
        }
