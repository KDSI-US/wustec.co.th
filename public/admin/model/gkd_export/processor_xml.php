  ...
  public function writeBody($fh, $config) {
    ...
    foreach ($products as $product) {
      ...
      foreach ($product as $k => $v) {
        if ($v !== '') {
          if (in_array($k, array('additional_images', 'product_filter', 'product_attribute', 'product_category', 'product_discount', 'product_special', 'product_option', 'related_product_id', 'related_product_name', 'store', 'products'))) {
            switch ($k) {
              ...
              case 'products': $subnode = 'product'; break;
            }
            ...
            if (is_array($v)) {
              ...
              foreach ($v as $arrKey => $arrValues) {
                if (is_array($arrValues)) {
                  ...
                  foreach ($arrValues as $subKey => $subVal) {
                    if (is_array($subVal) && $subKey == 'options') {
                      // options array                     
                      foreach ($subVal as $subVal2) {
                        $output .= "\n\t\t\t".'<option>';
                        foreach ($subVal2 as $optName => $optVal) {
                          $output .= "\n\t\t\t\t".'<'.$optName.'>'.str_replace('&', '&amp;', html_entity_decode($optVal, ENT_QUOTES, 'UTF-8')).'</'.$optName.'>';
                        }
                        $output .= "\n\t\t\t".'</option>';
                      }
                    }
                  ...
                  if (in_array($k, array('product_category'))) {
                    $output .= "\n\t\t".'<'.$subnode.' id="'.(int) $arrKey.'"><![CDATA['.str_replace('&', '&amp;', html_entity_decode($arrValues, ENT_QUOTES, 'UTF-8')).']]></'.$subnode.'>';
                  } else {
                    $output .= "\n\t\t".'<'.$subnode.'><![CDATA['.str_replace('&', '&amp;', html_entity_decode($arrValues, ENT_QUOTES, 'UTF-8')).']]></'.$subnode.'>';
                  }
                }
              }
              ...
              foreach ($values as $value) {
                $output .= "\n\t\t".'<'.$subnode.'>'.str_replace(array('&', '<![CDATA[', ']]>'), array('&amp;', '', ''), html_entity_decode($value, ENT_QUOTES, 'UTF-8')).'</'.$subnode.'>';
              }
              $output .= "\n\t".'</'.$k.'>';
            }
          } else {
            ...
            if (strpos($v, '<') !== false || strpos($v, '&') !== false || strpos($v, '<![CDATA[') !== false || strpos($v, ']]>') !== false) {
              $output .= "\n\t".'<'.$k.'><![CDATA['.str_replace(array('&', '<![CDATA[', ']]>'), array('&amp;', '', ''), $v).']]></'.$k.'>';
            } else if (strpos($v, '<') !== false || strpos($v, '&') !== false) {
              $output .= "\n\t".'<'.$k.'><![CDATA['.str_replace('&', '&amp;', $v).']]></'.$k.'>';
            } else {
              $output .= "\n\t".'<'.$k.'>'.str_replace('&', '&amp;', $v).'</'.$k.'>';
            }
