...
class ModelGkdExportDriverProduct extends Model {
  ...
  public function getItems($data = array(), $count = false, $asArray = false) {
    ...
    if (!empty($data['price_multiplier'])) {
      $select .= ", (SELECT (price * ".(float) $data['price_multiplier'].") AS price FROM " . DB_PREFIX . "product_special ps WHERE ps.product_id = p.product_id AND ps.customer_group_id = '" . (int)$special_price_group . "' AND ((ps.date_start = '0000-00-00' OR ps.date_start < NOW()) AND (ps.date_end = '0000-00-00' OR ps.date_end > NOW())) ORDER BY ps.priority ASC, ps.price ASC LIMIT 1) AS price_special";
    } else {
    ...
    if (!empty($filter_language)) {
      $this->config->set('config_language_id', $filter_language);
    }
    ...
    if (!empty($data['filter_category'])) {
      ...
      if (!empty($data['filter_category_exclude'])) {
        $sql .= " AND p2c.category_id NOT IN (" . $data['filter_category'] . ")";
      } else {
    ...
    if (!empty($data['filter_tag'])) {
      ...
      if (!empty($data['filter_model_exclude'])) {
        $sql .= " AND NOT (";
      } else {
        $sql .= " AND (";
      }
      ...
      $f = true;
      foreach (explode('|', $data['filter_model']) as $filterModel) {
        $sql .= ($f ? '' : ' OR ') . "p.model LIKE '%" . $this->db->escape($filterModel) . "%'";
        $f = false;
      }
    ...
    if (!empty($data['filter_ean'])) {
      ...
      $f = true;
      foreach (explode('|', $data['filter_ean']) as $filterModel) {
        $sql .= ($f ? '' : ' OR ') . "p.ean LIKE '%" . $this->db->escape($filterModel) . "%'";
        $f = false;
      }
    ...
    if (isset($data['filter_quantity_max']) && !is_null($data['filter_quantity_max']) && $data['filter_quantity_max'] !== '') {
    ...
    foreach ($query->rows as &$row) {
      if ($row['image'] && empty($data['param_image_path']) && substr($row['image'], 0, 4) != 'http') {
        $row['image'] = HTTP_CATALOG.'image/'.$row['image'];
      }
      ...
      if (empty($data['export_fields']) || in_array('additional_images', $data['export_fields'])) {
        ...
        if (!empty($data['param_image_inline']) && in_array('additional_images', $data['export_fields'])) {

          for ($i = 0; $i < 10; $i++) {
            $replace_field[] = 'image_'.($i+1);
          }

          $replace_key = array_search('additional_images', $data['export_fields']);
          array_splice($data['export_fields'], $replace_key, 1, $replace_field);

          $images = $this->getProductImages($row['product_id'], empty($data['param_image_path']), true);
          for ($i = 0; $i < 10; $i++) {
            $row['image_'.($i+1)] = isset($images[$i]) ? $images[$i] : '';
          }
        } elseif (!empty($data['param_image_inline'])) {
          $images = $this->getProductImages($row['product_id'], empty($data['param_image_path']), true);
          for ($i = 0; $i < 10; $i++) {
            $row['image_'.($i+1)] = isset($images[$i]) ? $images[$i] : '';
          }
      ...
      if (empty($data['export_fields']) || in_array('product_special', $data['export_fields'])) {
        $row['product_special'] = $this->getProductSpecials($row['product_id'], $asArray, $data);
        //if (!$asArray) {
        if (true) {
          $productSpecials = $this->getProductSpecials($row['product_id'], true, $data);
        ...
        foreach ($data['export_fields'] as $field) {
          if (empty($data['filter_language']) && in_array($field, array('name','description','tag','meta_title','meta_description','meta_keyword',/*'seo_keyword',*/'seo_h1','seo_h2','seo_h3','image_title','image_alt'))) {
            ...
            if (in_array($field, array('product_special'))) {
              ...
              if (empty($data['filter_language'])) {
                foreach ($this->langIdToCode as $lang) {
                  if (isset($row[$field.'_'.$lang])) {
                    $return[$i][$field.'_'.$lang] = $row[$field.'_'.$lang];
                    $valSet = true;
                  }
                }
              }
  ...
  public function getProductDescription($product_id, $store_id) {
    ...
    foreach ($query->rows as &$row) {
      if ($this->config->get('mlseo_enabled') || version_compare(VERSION, '3', '>=')) {
        ...
        } else if (version_compare(VERSION, '3', '>=') || ($this->config->get('mlseo_multistore') && $this->config->get('mlseo_ml_mode'))) {
        ...
        if (!in_array($key, array('language_id', 'product_id', 'seo_keyword'))) {
          if (isset($this->langIdToCode[$row['language_id']])) {
            if (isset($seoDescription[$row['language_id']][$key]) && trim(strip_tags($seoDescription[$row['language_id']][$key]))) {
              $res[$key.'_'.$this->langIdToCode[$row['language_id']]] = html_entity_decode($seoDescription[$row['language_id']][$key], ENT_QUOTES, 'UTF-8');
            } else {
              $res[$key.'_'.$this->langIdToCode[$row['language_id']]] = html_entity_decode($val, ENT_QUOTES, 'UTF-8');
            }
          }
        }
      }
  ...
  private function setSeoDescription(&$row, $store_id, $language_id) {
    ...
    if (!empty($seoDescription['meta_title'])) {
      $row['meta_title'] = html_entity_decode($seoDescription['meta_title'], ENT_QUOTES, 'UTF-8');
    }
    
    if (!empty($seoDescription['meta_description'])) {
      $row['meta_description'] = html_entity_decode($seoDescription['meta_description'], ENT_QUOTES, 'UTF-8');
    }
    ...
    if (isset($seoDescription['description']) && trim(strip_tags($seoDescription['description']))) {
      $row['description'] = html_entity_decode($seoDescription['description'], ENT_QUOTES, 'UTF-8');
    }
  ...
  public function getProductImages($product_id, $full_path, $asArray = false) {
    ...
    foreach ($query->rows as &$row) {
      if ($row['image']) {
        if ($full_path && substr($row['image'], 0, 4) != 'http') {
  ...
  public function getProductSpecials($product_id, $asArray = false, $data = array()) {
    
    if (!empty($data['price_multiplier'])) {
      $query = $this->db->query("SELECT *, (price * ".(float) $data['price_multiplier'].") AS price FROM " . DB_PREFIX . "product_special WHERE product_id = '" . (int)$product_id . "' ORDER BY priority, price");
    } else {
