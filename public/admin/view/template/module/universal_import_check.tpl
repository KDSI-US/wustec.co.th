<?php function rowSatusIcon($status, $colored = true) {
  switch ($status) {
    ...
    case 'info': $icon = 'exclamation-circle'; break;
  }
  echo '<i class="fa fa-'.$icon.' '.($colored ? $status : '').'"></i>';
} ?>
        ...
        <?php if (isset($simu['row_status'])) { ?>
          ...
          <tbody>
             <?php if (!empty($preview_errors[$row])) { ?>
              <?php foreach ($preview_errors[$row] as $log_status => $log_messages) { ?>
                <tr>
                  <td><?php echo $_language->get('text_simu_'.$log_status); ?></td>
                  <td>
                    <div class="alert alert-<?php echo $log_status == 'info' ? 'info' : 'danger'; ?>" style="margin-bottom:0">
                      <?php foreach ($log_messages as $err_msg) { ?>
                      <?php rowSatusIcon($log_status, false); ?> <?php echo $err_msg; ?><br/>
                      <?php } ?>
                    </div>
                  </td>
                </tr>
