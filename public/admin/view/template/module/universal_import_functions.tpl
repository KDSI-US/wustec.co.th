...
function getXfn($type, $values, $count, $columns, $profile, $_language, $languages, $vars) {
  ...
  if ($type == 'oc_currency') {
    ...
    */
    ...
    $xfn .= '  </td><td>';
    $xfn .= '    <select class="'.(in_array($vars['filetype'], array('xml', 'json')) ? 'selectize' : 'form-control').'" name="extra_func['.$count.']['.$type.'][field]" disabled="disabled">';
  ...
  if (in_array($type, array('if_table', 'wholesale'))) {
      ...
      $xfn .= '  </td><td>';
        $xfn .= '    <select style="width:100%" class="'.(in_array($vars['filetype'], array('xml', 'json')) ? 'selectize' : 'form-control').'" name="extra_func['.$count.']['.$type.'][field]" disabled="disabled">';
        foreach ($columns as $key => $row) {
          $xfn .= '          <option value="'.$key.'" '. ((isset($values['field']) && $values['field'] == $key) ? 'selected' : '') .'>'. $row .'</option>';
        }
        if (!empty($values['field']) && !in_array($values['field'], array_keys($columns)) && substr($values['field'], 0, 14) != '__extra_field_') {
  ...
  // Target field only
  } else if (in_array($type, array('nl2br', 'strip_tags', 'strlen', 'html_encode', 'html_decode', 'uppercase', 'lowercase', 'ucfirst', 'ucwords', 'remote_content', 'to_float', 'urlencode', 'urldecode', 'jsonencode', 'jsondecode'))) {
    //$xfn .= '  <tr><td>';
    ...
    if ($type == 'jsondecode') {
      ...
      $xfn .= '<span data-toggle="tooltip" title="'.$_language->get('source_api_field_i').'">'.$_language->get('source_api_field').'</span>';
      $xfn .= '  </td><td>';
      $xfn .= '    <input type="text" name="extra_func['.$count.']['.$type.'][path]" value="'.(isset($values['path']) ? $values['path'] : '').'" disabled="disabled" class="form-control" placeholder="parameters"/>';
      $xfn .= '  </td></tr><tr><td>';
    }
    //$xfn .= '  </td></tr>';
    ...
    if (in_array($type, array('append', 'prepend', 'add', 'subtract', 'replace'))) {
      ...
      $xfn .= '    <select class="'.(in_array($vars['filetype'], array('xml', 'json')) ? 'selectizeCustom' : 'form-control').' xfnFieldVal" name="extra_func['.$count.']['.$type.'][fieldval]" disabled="disabled">';
      ...
      if (!empty($values['fieldval']) && !in_array($values['fieldval'], array_keys($columns)) && substr($values['fieldval'], 0, 14) != '__extra_field_') {
        $xfn .= '          <option value="'.$values['fieldval'].'" selected>'.$values['fieldval'].'</option>';
      }
      if (!empty($profile['extra_fields'])) {
        foreach (explode(',', $profile['extra_fields']) as $i => $extra_field_name) {
    ...
    switch ($type) {
      ...
      case 'date_convert': $for = $_language->get('text_convert_from'); $set = '<span data-toggle="tooltip" title="'.$_language->get('text_init_date_i').'">'.$_language->get('text_init_date').'</span>'; break;
    ...
    if (!in_array($type, array())) {
      ...
      if ($type == 'skip_full__') {
        ...
        if (in_array($profile['import_type'], array('product', 'product_update'))) {
          $dbFieldsTypes = array('name', 'description', 'model', 'sku', 'upc', 'ean', 'jan', 'isbn', 'mpn', 'location', 'quantity', 'subtract', 'price', 'special', 'status', 'manufacturer_id', 'date_added', 'date_modified', 'import_batch');
    ...
    if (in_array($type, array('date_convert'))) {
      ...
      $xfn .= '  </td></tr><tr><td>';
      $xfn .= '    <span data-toggle="tooltip" title="'.$_language->get('text_alter_date_i').'">'.$_language->get('text_alter_date').'</span>';
      $xfn .= '  </td><td>';
      $xfn .= '    <input type="text" name="extra_func['.$count.']['.$type.'][delay]" value="'.(isset($values['delay']) ? $values['delay'] : '').'" disabled="disabled" placeholder="+15 days, +6 months, +1 year" class="form-control" '.(isset($values['fieldval']) && $values['fieldval'] !== '' ? 'style="display:none"' : '').'/>';
...
function extraDisableUpdate($columns, $profile, $_language, $languages, $vars) {
  $gkhtab = $gkhdiv = 0;
?>
  <div class="gkdwidget gkdwidget-color-blueLight clearfix">
    <header role="heading">
      ...
      <ul class="nav nav-tabs pull-left in">
        <li><a data-toggle="tab" href="#gkhelpxfndc<?php echo $gkhtab++; ?>"><span class="hidden-mobile hidden-tablet"><?php echo $_language->get('tab_disable_update'); ?></span></a></li>
      </ul>
    </header>
    <div class="gkdwidget-container" style="display:none">
      <div class="tab-content">
        <div class="tab-pane" id="gkhelpxfndc<?php echo $gkhdiv++; ?>"><?php echo $_language->get('info_disable_update'); ?></div>
      </div>
    </div>
  </div>
  
  <div class="form-group">
    <label class="col-sm-2 control-label"><?php echo $_language->get('entry_disable_update'); ?></label>
    <div class="col-md-8">
      <input class="form-control" type="text" name="disable_update" value="<?php echo isset($profile['disable_update']) ? $profile['disable_update'] : ''; ?>" placeholder="price, quantity"/>
    </div>
  </div>
  
  <div class="form-group">
    <label class="col-sm-2 control-label"><?php echo $_language->get('entry_disable_update_empty'); ?></label>
    <div class="col-md-8">
      <input class="form-control" type="text" name="disable_update_empty" value="<?php echo isset($profile['disable_update_empty']) ? $profile['disable_update_empty'] : ''; ?>" placeholder="price, quantity"/>
    </div>
  </div>
<?php
}
...
$xfn_names = array(
  ...
  'html' => array('strip_tags', 'nl2br', 'html_encode', 'html_decode', 'urlencode', 'urldecode', 'jsonencode', 'jsondecode'),
...
<div class="alert alert-default">
  <input class="switch" type="checkbox" data-label="<?php echo $_language->get('text_verbose'); ?>" name="xfn_verbose" id="xfn_verbose" value="1" <?php echo (!empty($profile['xfn_verbose'])) ? 'checked="checked"' : ''; ?>/>
</div>
...
$('#extraFuncs .selectizeCustom').selectize({create: function(input) {
  return {
    value: input,
    text: input
  }
