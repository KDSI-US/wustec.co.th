...
<div class="panel panel-default">
  ...
  <!--<div id="stores" class="form-inline" <?php if (version_compare(VERSION, '2', '>=') && 0) echo 'class="v2"'; ?>>
    	...
    	<li class="active"><a href="#tab-0" data-toggle="tab"><i class="fa fa-download"></i><?php echo $_language->get('text_tab_0'); ?></a></li>
			<li><a href="#tab-1" id="tabExport" data-toggle="tab"><i class="fa fa-upload"></i><?php echo $_language->get('text_tab_1'); ?></a></li>
          ...
          <div class="form-group">
            ...
            <div class="col-sm-10">
              <select name="<?php echo $prefix; ?>cron_log" id="<?php echo $prefix; ?>cron_log" class="form-control">
                <?php foreach (array('all', 'min', 'error', 'off') as $log_type) { ?>
...
var getOptionFields = function(init) {
  $('#optionFields tbody').html('<tr><td colspan="3" style="text-align:center"><img src="view/universal_import/img/loader.gif" alt=""/></td></tr>');
  if (init !== 1) {
    init = '';
  }
  
  $.post('index.php?route=module/universal_import/get_option_fields&init='+init+'&<?php echo $token; ?>', $('#import-form').serialize(), function(data) {
    $('#optionFields tbody').html(data);
  });
}

$('body').on('click', 'button.get-option-fields', getOptionFields);
...
$('body').on('click', '.add-attribute-field', function (e) {
  ...
  extra.find('[disabled]').each(function(i) {
    $(this).attr('name', $(this).attr('name').replace('attribute_fields[_extra_]', 'attribute_fields['+attrRow+']'));
    $(this).attr('name', $(this).attr('name').replace('attribute_fields_default[_extra_]', 'attribute_fields_default['+attrRow+']'));
  });
...
$('body').on('click', '.add-function', function (e) {
  ...
  $('#extraFuncs tr[class^="xfn-"]:last .selectizeCustom').selectize({create: function(input) {
  return {
    value: input,
    text: input
  }
...
$('body').on('keyup change', '.filters input', function(){getTotalExportCount();});
