...
<div class="tab-content">
  ...
  <?php if (in_array($type, array('product', 'category', 'manufacturer')) && !$update) { ?>
    <div class="tab-pane" id="tab-setting-image">
      ...
      <div class="form-group">
        <?php inputField('image_number', $profile, $_language, 'text'); ?>
      </div>
  ...
  <?php if (in_array($type, array('product'))) { ?>
    <div class="tab-pane" id="tab-setting-product">
      ...
      <div class="form-group">
        <label class="col-sm-2 control-label"><span data-toggle="tooltip" title="<?php echo $_language->get('entry_attribute_generate_i'); ?>"><?php echo $_language->get('entry_attribute_generate'); ?></span></label>
        <div class="col-sm-4">
          <input class="switch" type="checkbox" name="attributes_from_options" id="defaults_attributes_from_options" data-label="<?php echo $_language->get('entry_option'); ?>" value="1" <?php if (!empty($profile['attributes_from_options']) || (!isset($profile['attributes_from_options'])) && !empty($default)) echo 'checked'; ?>/>
        </div>
      </div>
      <div class="form-group">
  ...
  <?php if (!in_array($type, array('order_status'))) { ?>
    <div class="tab-pane" id="tab-setting-remove">
      <div class="col-md-offset-2">
        <div class="gkdwidget gkdwidget-color-blueLight clearfix">
          <header role="heading">
            <i class="icon fa fa-info-circle fa-2x pull-left" title="<?php echo $_language->get('info_help'); ?>"></i>
            <ul class="nav nav-tabs pull-left in">
              <li><a data-toggle="tab" href="#gkhelps2<?php echo $gkhtab++; ?>"><span class="hidden-mobile hidden-tablet"><?php echo $_language->get('text_delete_settings'); ?></span></a></li>
            </ul>
          </header>
          <div class="gkdwidget-container" style="display:none">
            <div class="tab-content">
              <div class="tab-pane" id="gkhelps2<?php echo $gkhdiv++; ?>"><?php echo $_language->get('info_remove'); ?></div>
            </div>
          </div>
        </div>
      </div>
      ...
      <?php
       $delete_types = array(
        ...
        if ($type == 'product') {
          ...
          $delete_modes['zero_opt'] = $_language->get('text_delete_zero_opt');
          $delete_modes['disable_zero'] = $_language->get('text_delete_disable_zero');
          $delete_modes['disable_zero_opt'] = $_language->get('text_delete_disable_zero_opt');
        }
      ...
      <div class="col-sm-6">
        ...
        <?php } ?>
        <div class="delete_action clearfix">
        ...
        <div class="delete_action clearfix" style="margin-top:7px">
          <label class="col-sm-3 control-label"><?php echo $_language->get('entry_delete_notempty'); ?></label>
          <div class="col-sm-9">
            <select class="form-control" name="delete_notempty">
              <?php foreach (array('', '1') as $val) { ?>
                <option value="<?php echo $val; ?>" <?php if (isset($profile['delete_notempty']) && $profile['delete_notempty'] == $val) echo 'selected'; ?>><?php echo $_language->get('entry_delete_notempty_'.$val); ?></option>
              <?php } ?>
            </select>
          </div>
        </div>
    ...
    <div class="col-md-offset-2">
      <div class="gkdwidget gkdwidget-color-blueLight clearfix">
        <header role="heading">
          <i class="icon fa fa-info-circle fa-2x pull-left" title="<?php echo $_language->get('info_help'); ?>"></i>
          <ul class="nav nav-tabs pull-left in">
            <li><a data-toggle="tab" href="#gkhelps2<?php echo $gkhtab++; ?>"><span class="hidden-mobile hidden-tablet"><?php echo $_language->get('text_extra_fields'); ?></span></a></li>
          </ul>
        </header>
        <div class="gkdwidget-container" style="display:none">
          <div class="tab-content">
            <div class="tab-pane" id="gkhelps2<?php echo $gkhdiv++; ?>"><?php echo $_language->get('info_extra'); ?></div>
          </div>
        </div>
      </div>
    </div>
