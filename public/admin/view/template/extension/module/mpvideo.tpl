  ...
  <div class="container-fluid">
    <div class="panel panel-default">
      <div class="panel-heading">
        ...
        <button type="submit" form="form-mpvideo" class="btn btn-success"><i class="fa fa-save"></i> <?php echo $button_save; ?></button>
        <a href="<?php echo $cancel; ?>" class="btn btn-default"><i class="fa fa-times"></i> <?php echo $button_cancel; ?></a></div>
      </div>
      <div class="panel-body">
        <form action="<?php echo $action; ?>" method="post" enctype="multipart/form-data" id="form-mpvideo" class="form-horizontal">
          ...
          <div class="form-group required">
            ...
            <div class="col-sm-10">
              ...
              <div class="input-group">
                ...
                <input type="text" name="video_description[<?php echo $language['language_id']; ?>][title]" value="<?php echo isset($video_description[$language['language_id']]) ? $video_description[$language['language_id']]['title'] : ''; ?>" placeholder="<?php echo $entry_title; ?>" class="form-control" />
              </div>
          ...
          <div class="form-group required">
            <label class="col-sm-2 control-label" for="input-video"><span data-toggle="tooltip" title="<?php echo $help_video; ?>"><?php echo $entry_video; ?></span></label>
							<div class="col-sm-10">
								<input type="text" name="video_name" value="<?php echo $video_name; ?>" placeholder="<?php echo $entry_video; ?>" id="input-video" class="form-control" />
                  <input type="hidden" name="gallery_id" value="<?php echo $gallery_id; ?>" />
									<?php if ($error_video_name) { ?>
										<div class="text-danger"><?php echo $error_video_name; ?></div>
									<?php } ?>
							</div>                
           </div>
          <div class="form-group">
...
$('input[name=\'video_name\']').autocomplete({
	'source': function(request, response) {
		$.ajax({
			url: 'index.php?route=extension/mpphotogallery/gallery/autocomplete&<?php echo $get_token; ?>=<?php echo $token; ?>&filter_name=' +  encodeURIComponent(request),
			dataType: 'json',
	...
	'select': function(item) {
		$('input[name=\'video_name\']').val(item['label']);
